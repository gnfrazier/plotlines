...
* **K10 / FR86, FR95, FR101.** Every licensed data source's attribution, shown
  ...
  basemap's line), Nominatim's own display-attribution obligation for
  ``/geocode`` results (issue #296 — a different usage policy again, unmet
  until now), and a credit line for **every loaded plugin layer** (FR101,
...
#: always ships and owes ODbL, the routing graph — built from the same
...
#: accident (issue #269, addendum L6), and `/geocode` owes Nominatim's own
#: display-attribution obligation (issue #296, addendum P2), unmet until
#: now and distinct from the other three. Enumerated here so the build gate
#: can assert their presence independently of the dynamic per-layer set.
_STATIC_ATTRIBUTIONS = ("elevation", "basemap", "graph", "geocode")
...
def about_attributions(registry) -> list[dict]:
    ...
    routing graph (ODbL, issue #269), then Nominatim's display credit
    (issue #296) — the four static obligations, shown together because each
    is a *separate* obligation even where more than one shares a licence —
    then every ready layer's credit (FR101), built-ins before plugins, each
    alphabetical.
    """
    lines: list[dict] = [
...
def assert_about_attribution_complete(registry) -> list[dict]:
    ...
    *will* be shown; raises
    ...
    # The static half: elevation, basemap, the routing graph, and `/geocode`
    # always ship, so their credit lines must always be non-empty. A drift
    # here (a constant emptied by a bad merge) is exactly the failure this
