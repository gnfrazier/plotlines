...
| **SPIKE-I** local-extract parity | Done 2026-09-13 — **RESCOPE, in the clip not the graph**: transport swap is exact, pyrosm rejected; the mirror clip is O(extract) not O(bbox) (#375) and the two-extract path dies (#376) |
| **SPIKE-J** native-dependency freeze matrix | Done 2026-09-15 — **PARITY ×4**: pyosmium freezes on Linux/Windows/macOS x86/macOS arm at +3.4–4.5 MB; no-GPL check passes; A5 confirmed. Phase 3 (#272) may move osmium into the sidecar |
...
- **The mirror now has a clip endpoint; reachability is still the open piece.** Phase 1 landed the host side — `deploy/mirror/` (tree, Caddyfile, licence artifacts, `geofabrik_pull.py`: at most daily, conditional, identified, backs off), the WNC-corridor basemap stand-in under an honest build id, and `tiles/mirror_state.py` surfacing mirror age on `/health` with a named pin owner. #261 pointed the sidecar at the mirror by name (`classify_upstream` returns `MIRROR` for it). #262 (2026-09-12) added the one dynamic endpoint Q1-C calls for — `service/plotlines_service/mirror_clip.py` (bbox in, clipped `.osm.pbf` out, pyosmium-only per addendum L1, deliberately kept outside `core/` and the frozen sidecar's import graph — SPIKE-J has since measured that pyosmium freezes fine on all four targets, so that exclusion is now Phase 3's to lift, not a risk), `service/Dockerfile.mirror-clip`, and the Caddy `reverse_proxy /clip*` route in `deploy/mirror/Caddyfile` — but it hasn't been rehearsed against the live Pi yet (that needs a real multi-region pull; the tests are hermetic synthetic-fixture coverage), **so the wall-time / output-size / peak-RSS numbers Q1-C and Q6 rest on have not actually been taken**. The reachability posture is **decided** (#263, 2026-09-12): split — static files open, `/clip` gated by a shared `X-Plotlines-Client-Key` plus a per-IP rate ceiling, explicitly not accounts. `/clip` also carries its own ODbL notice (#364), because Caddy's `reverse_proxy` matcher terminates before `file_server` and a clip output is a *Derivative* Database, so a caller there never reads `COPYRIGHT.txt`. SPIKE-I (#265, Phase 2) ran 2026-09-13 against it: graph parity exact, clip cost and the two-extract path both fail their bands (#375, #376).
