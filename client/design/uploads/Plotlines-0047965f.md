...
Creating a good story has an arch to it, establishing the characters, key points that make things interesting, some struggle or obstacle to over come, then a conclusion where the characters reach a stopping point. 
...
### Story C22: Interactive Auto-Updating Cue Sheet HUD (Primary Character View)

**As a** Character on a trip

**I want** the mobile app’s primary execution view to be a live, auto-scrolling cue sheet HUD pinned to my current route progress

**So that** I have instant visibility into upcoming turns, distance-to-next-milestone, and active segment metrics at a glance without having to navigate map layers.

**Acceptance Criteria:**

- Opening an active trip defaults directly to the **Interactive Cue Sheet HUD**, displaying a real-time progress bar (e.g., _Mile 14.2 of 38.5_).
    
- The cue sheet automatically highlights and scrolls to the immediate next upcoming cue, displaying distance remaining to that specific point.
    
- The header bar provides dynamic readouts for total remaining distance, remaining elevation gain, and estimated arrival time (ETA) based on current pace.
    
- Tapping any cue row expands a rich detail card showing attached author notes, photos, hazard badges, or transition instructions.
    
- A floating button or gesture allows seamless, instant toggling between the Cue Sheet HUD and the Vector Map View.
    

### Story C23: Dynamic Recalculation of Cue Distances & ETAs

**As a** Character following a daily itinerary

**I want** cumulative mileage markers and estimated arrival times across all upcoming cues to dynamically recalculate based on my actual GPS location and moving pace

**So that** my cue sheet remains accurate even if we start late, pause for extra rest, or take a temporary detour.

**Acceptance Criteria:**

- Passing or missing a scheduled cue automatically advances the active cue marker to the next downstream node.
    
- If a character is ahead of or behind the planned schedule, estimated arrival times for all remaining cues (including timed events, rest stops, and regroup points) update live.
    
- If a character toggles to an active alternate bypass route (Story C7 / Story 36), the cue sheet dynamically inserts the bypass cue steps and deletes the bypassed route cues without requiring a full app reload.
    

### Story C24: Hands-Free & Glove-Friendly Cue Navigation

**As a** Character riding a bike, hiking with poles, or paddling with gloves on

**I want** large-target controls, auto-advance logic, and high-visibility typography on the cue sheet HUD

**So that** I can easily inspect upcoming cues and interact with the screen using single taps or swipes.

**Acceptance Criteria:**

- Active cue cards feature oversized, high-contrast typography and directional arrow icons optimized for outdoor daylight legibility.
    
- Interacting with cues (expanding notes, marking a regroup point as reached, or stepping through in Manual Odometer Mode) utilizes large touch targets (minimum 48x48 dp) compatible with gloved hands.
    
- Optional device volume-button triggers or swipe gestures allow stepping forward/backward through cue points without precise touch-screen targeting.
...
### Story S9: Configure User Language Preference
As an Any User (Author or Character)
I want to select my preferred application language in the system settings
So that all UI text, navigation controls, map labels, and system notifications are displayed in my native language.
Acceptance Criteria:
 * User settings interface provides a "Language" selection menu displaying available supported languages in their native scripts (e.g., English, Español, Français, Deutsch).
 * Defaults to the device's system locale on first launch if supported; falls back to English if the system locale is unavailable.
 * Changing language updates all static UI elements, button labels, navigation prompts, and dialogs dynamically without requiring an app restart.
 * Selected language preference is saved to the local profile and synced across devices when signed in.

### Story D1: Implement ARB-Based Localization (l10n) Framework
As a Developer
I want to establish a standardized localization pipeline using Flutter's official flutter_localizations and Application Resource Bundle (.arb) files
So that adding new languages or updating translated strings is streamlined, type-safe, and decoupled from core application logic.
Acceptance Criteria:
 * All hardcoded UI strings are extracted out of Dart widgets into template .arb files (e.g., app_en.arb, app_es.arb) stored in a dedicated /l10n directory.
 * Flutter’s code generation (generate: true in pubspec.yaml) auto-generates type-safe AppLocalizations classes accessible via AppLocalizations.of(context).
 * Support for parameter interpolation (e.g., {distance} miles remaining), pluralization rules, and date/time/number formatting via the intl package is fully configured.
 * Missing translation keys fall back gracefully to the primary base locale (app_en.arb) during build time, outputting clear compiler warnings for unlocalized strings.
 * 
