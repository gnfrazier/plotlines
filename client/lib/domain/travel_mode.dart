/// FR10/B1, FR144/N0 — the real, offerable travel-mode **categories**, shared
/// by the trip-creation mode-declaration prompt (`trip_mode_prompt.dart`), New
/// Route's per-segment mode picker, the passage inspector's mode picker, and
/// the layer picker's "defaults derived from" label, so all of them always
/// agree on the same list and labels.
///
/// This mirrors `core/plotlines_core/multimodal/modes.py`'s registry, which is
/// where a category's weight profile and domain parameters live (FR130: adding
/// one is a `WeightProfile` entry plus domain parameters, never a parallel
/// scorer). The client half carries only what a picker needs — the list, the
/// tier, and the label — because Domain stays free of solver concerns
/// (ARCH §10.1).
///
/// **Issue #315:** the list is the traversal *categories* now.
/// `mountain_biking` / `packrafting` / `riverboarding` left the enum and
/// became disciplines (`discipline.dart`) — the second axis, chosen per
/// passage. `legacy_mode.dart` maps the old spelling forward on read.
///
/// **FR109/O4 guard:** climbing, canyoneering, and jumaring are stations, not
/// travel modes, and must never appear here — see
/// `travel_mode_test.dart`'s regression check.
library;

/// FR10's traversal categories, in the order the PRD states them: cycling,
/// hiking, paddling, cross-country skiing (the "Ski" category), and driving.
/// Driving is a **routed** mode (FR29 [AMENDED v2.0]), not a note — the last
/// mile to the trailhead is often the day's worst.
///
/// Canonical order — used wherever a single "primary" mode has to be picked
/// out of a set deterministically (e.g. New Route's default per-segment
/// mode), rather than relying on a `Set`'s insertion order.
const List<String> kTraversalModes = [
  'cycling',
  'hiking',
  'paddling',
  'cross_country_skiing',
  'driving',
];

/// Alias for call sites that mean "a broad travel category" rather than "a
/// wire mode value" — issue #315 makes the distinction load-bearing (the
/// trip declares categories; a passage picks a discipline under one).
const List<String> kTravelCategories = kTraversalModes;

/// The three PRD §10 names as first-class at MVP. The rest are authorable
/// today and absorbed by `WeightProfile` config; what "first-class" buys is a
/// tuned profile and a router that has been measured against real routes, not
/// the mode's existence.
const List<String> kFirstClassTravelModes = ['cycling', 'hiking', 'paddling'];

/// FR29's other half — train, shuttle and flight legs are **authored notes**
/// carrying carrier, identifiers and scheduled times, never solved. A payload
/// mode value that is deliberately not a traversal mode.
const List<String> kTransportNoteModes = ['transit'];

/// Every value `$defs/travel_mode` accepts. Traversal categories first, then
/// the note modes, matching `multimodal.modes.all_mode_keys()`.
const List<String> kTravelModes = [...kTraversalModes, ...kTransportNoteModes];

/// True for a mode that produces a solved route with metrics and a cue sheet.
/// False for `transit`, whose timing is an authored schedule.
bool isRoutedMode(String mode) => kTraversalModes.contains(mode);

/// True for one of the three MVP-tuned modes.
bool isFirstClassMode(String mode) => kFirstClassTravelModes.contains(mode);

/// The label for a wire `travel_mode` value (`kTravelModes`). For a broad
/// category chip, prefer [travelCategoryLabel] — same string today for the
/// five categories, but named for what it is.
String travelModeLabel(String mode) => switch (mode) {
      'cycling' => 'Ride',
      'paddling' => 'Paddle',
      'hiking' => 'Hike',
      'cross_country_skiing' => 'Ski',
      'driving' => 'Drive',
      'transit' => 'Transit',
      _ => mode,
    };

/// The label for a broad travel category (`kTravelCategories`) as the
/// trip-creation control presents it — the category, not a specific
/// discipline under it (issue #315).
String travelCategoryLabel(String category) => switch (category) {
      'cycling' => 'Cycle',
      'hiking' => 'Foot',
      'paddling' => 'Paddle',
      'cross_country_skiing' => 'Ski',
      'driving' => 'Drive',
      _ => travelModeLabel(category),
    };

/// FR120/D41, issue #208 — the OSMnx `network_type` a segment in [mode] must
/// route against. Mirrors `network_type_for` in
/// `core/plotlines_core/multimodal/modes.py` (and each `TraversalMode
/// .network_type` row): cycling takes `bike`, hiking takes `walk`, driving
/// takes `drive`, and the water/snow categories take OSMnx's widest `all`
/// network (their real routing graph is a later Leg; `all` is the honest
/// placeholder the registry already uses).
///
/// A discipline never changes this — road / gravel / mountain all route on
/// `cycling`'s `bike` graph (issue #315, and #208 for the shared MTB graph).
///
/// This matters because `RoutingClient.ensureRegion` builds the graph for
/// exactly the `network_type` it is handed — `region_key` is
/// `(bbox, network_type, ruleset)` — so a `driving` passage ensured against
/// the default `bike` graph routes a car down singletrack and never sees the
/// `drive` graph's dropped `track`/`service` ways (SPIKE-E, issue #171).
///
/// An unknown or note mode (`transit`, or a plugin mode this build has never
/// heard of) falls through to `bike`, exactly as `network_type_for` does.
String networkTypeForMode(String mode) => switch (mode) {
      'cycling' => 'bike',
      'hiking' => 'walk',
      'driving' => 'drive',
      'paddling' || 'cross_country_skiing' => 'all',
      _ => 'bike',
    };
