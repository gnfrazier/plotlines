...
**The ruleset is data, not code.** Per-tag rules — `historic=*` sub-weighted by value, `natural=tree` requiring `denotation`, `leisure=park` requiring a name or area threshold — live in a versioned config file, because they will need regional tuning and tuning must not be a code release. This is the same "themes are data" discipline as `WeightProfile` (D5).
...
**Unproven, and stated as such:** the initial ruleset values are guesses until measured. SPIKE-A (§18) calibrates them against real extracts in NC, WI, and SoCal. Under-filtering floods the map; over-filtering hides the castle. Both fail visibly, which is at least a fast feedback loop.
...
**Cost is unmeasured.** SPIKE-B (§18) measures runtime and tunes ranking against a realistic multi-day bbox. If it proves expensive, the mitigation is the same as `/segments/envelope`'s: it is a separate, cacheable endpoint (§7.2), so it can be slow without making anything else feel slow.
---
...
**`terrain_technicality` remains unproven**, not proven: SPIKE-04 was a paddling spike and never measured `sac_scale` / `mtb:scale` density. **v2.0 sharpens this into an action**: PRD FR14b requires reading published grading where it exists, and SPIKE-C (§18) measures whether it does. v1.0 generalized SPIKE-04's whitewater verdict to all technical terrain without testing it.
...
| **Privacy statement** | **Required, all platforms** ★ | **Elevated from optional in v2.0** (FR138). Reachable from About on **every** surface including Web guest and the share-token reading view (§10.3), which reach people who may have no account at all. States plainly: what is on the device vs. the server; that **reveal is a product guarantee, not a security boundary** (FR64a); what arrival sharing does and does not do, defaulting to nothing shared; **that an Author may keep private notes about Characters, visible only to that Author, persisting across trips, deletable on request** (FR135, FR135a); and that guest sessions leave no server-side trace (P4). **Author notes are what made this required** — they are the first data Plotlines holds *about* a person *by someone other than that person*. Not legal boilerplate; it says what is true, briefly, in the app's voice. |
...
| A16 | **Desktop memory ~1 GB with a basemap on screen** | Medium | Budget near 1 GB; re-measure on a release build. Lever: tile-cache bounds. **v2.0 adds candidate rendering to the same screen** — a dense bbox draws thousands of markers over the basemap, so A16 should be re-measured *with candidates displayed*, not just with a route. |
...
| **A20** ★ | **Notability ruleset is unvalidated, and cluster quality depends entirely on it.** Over-filtering hides the castle; under-filtering floods the map and makes proposals noise | **HIGH** | The ruleset is versioned config, not code (§4.3), so tuning is not a release. **SPIKE-A calibrates against NC/WI/SoCal before Leg 2.5 ships.** Golden candidate sets (§15.1) make a regression visible. Failure is at least loud rather than silent. |
| **A21** ★ | **Co-location analysis cost is unmeasured** over a realistic multi-day bbox with many layers live | Medium | **SPIKE-B measures and tunes ranking.** Structural mitigation already in place: it is a separate cacheable endpoint (§8.2), so it can be slow without making anything else feel slow. |
...
| **A23** ★ | **Candidate extraction overloads public Overpass.** It is a heavier query than graph building, which SPIKE-04 §8 already could not complete without tiling and retries | Medium | Bbox-scoped on-demand cache (§4.2); prefer local extracts for repeatedly-used regions (§12); tile-and-retry as the baseline access pattern, not a fallback. |
...
| **A26** ★ | **The anonymous web reading surface leaks reader identity or trip content.** A bearer token in a URL, plus access logs, plus a reveal model with no home for an accountless reader | Medium | **SPIKE-F / Q17**, gated to the web leg and final before any web presentation. Structural mitigation available now: the share token is already revocable and scoped to one trip's reveal-filtered content (§10.3), and web guests persist no preferences (§11.2), which is one fewer fingerprinting surface |
...
|---|---|---|---|
| **D34** | **`/health` reports per-capability readiness; layer/POI extraction is ordered ahead of elevation enrichment** *(breaking — B1)* | PRD FR121 has the Author curating during a minutes-long enrichment. A single readiness flag either blocks the session or lies. Ordering is a reordering of existing startup work, not new work (SPIKE-D confirms) | One flag (blocks authoring for minutes — the thing the requirement exists to prevent); reporting ready-when-up (lies about routing, and the first solve fails or hangs); running elevation first because it always has (an ordering nobody chose, now load-bearing against us) |
...
| **Q11** ★ | **Notability ruleset values** — the per-tag qualification rules and `historic=*` sub-weighting are guesses until measured, and cluster quality depends entirely on them | **SPIKE-A**, before Leg 2.5 ships |
| **Q12** ★ | **Cluster ranking function** — how salience and tightness trade off; whether corridor proximity should dominate once a route exists; what a "reviewable count" is for a 200 km bbox | **SPIKE-B** |
| **Q13** ★ | **Non-whitewater difficulty coverage** — `sac_scale`, `trail_visibility`, `mtb:scale*`, `piste:difficulty`. **v1.0 generalized SPIKE-04's whitewater verdict to all technical terrain without testing it**; FR14b reopens it | **SPIKE-C**, before committing to Author-declared difficulty as the only path |
...
| **Q15** ★ | **Candidate rendering at bbox scale** — a dense trip bbox may draw thousands of markers over a basemap already costing ~1 GB (A16). Clustering-for-display, zoom thresholds, or salience-gated rendering? | Before the curation workspace UI; re-measure A16 with candidates on screen |
| **Q16** ★ | **The utility-amenity layer is incomplete.** The OSM attribute mapping does not contain `amenity=toilets` at all, nor `cafe`/`restaurant`/`pharmacy`/`shower`. **FR104's "toilet + water + shelter" provision cluster cannot be computed from the mapping as written** | Before Leg 2.5 — a mapping pass, not a spike |
| **Q17** ★ | **Anonymity of the Character-facing web reading surface** (FR132, §10.3). Three strands: a share token in a URL leaks through referrers, browser history, and access logs; CDN and server logs are a retention surface for readers who never consented to an account; and **reveal state has nowhere to live for an anonymous reader** — no account, no device GPS, nothing to fire a reveal. Likely resolution is that an anonymous share-token reader sees the **always-visible set only** and revealed content requires an account, but that is a decision to make, not an assumption to inherit | **SPIKE-F.** Joint design, security, and product call. **Not an MVP blocker; gated to the web/hosted leg and final before any web presentation ships** |
