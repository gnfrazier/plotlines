...
**v2.0 adds extension paths, and they are different** (PRD FR130). A new **traversal category** is a `WeightProfile` entry plus its domain parameters — the mechanism M1 already describes. A new **discipline** under a category (issue #315: road / gravel / mountain, and so on — `multimodal/disciplines.py`) is the same shape one axis down: a `WeightProfile` a solve resolves from `(category, discipline)`, no scorer branch, no new graph or legality (it inherits the category's). `mountain_biking` / `packrafting` / `riverboarding` were categories through v2.0 and are now disciplines; `multimodal/legacy.py` maps the old spelling forward and `_resolve_profile` resolves a discipline name ahead of the mode fallback. A new **station activity** (climbing, canyoneering, jumaring) needs **no routing change at all**: it is an activity-type config entry consumed by `content/` and day timing, never by the scorer. Filing station activities as "more modes" is what made nine modes look like a scoping decision when it was a modeling gap.
...
**Wired into the compose path (issue #202).** #200 stopped at the primitive; the callers now use it. `trips.compose.solver_profile_for_day` resolves the effective Author-facing profile for a passage **most-specific-wins** — `Segment.weights` → `Day.weights` → `Trip.default_weights`, whole-profile, an unset level falling through — and runs the winner through `solver_profile_from_author`. `trips.spine.spine_solver_profile` is the compose companion to `spine_waypoints`: it carries that solver profile into `generate_segment` so a composed passage is *flavoured* by the Author's dials while distance stays an output (no band, no `target_distance`, §7.7), and it forces `interest` back to neutral because the salience bias is explore-only. Explore uses the same `solver_profile_from_author` for its seed profile, so the two postures never diverge on how a stored profile becomes a solver one. **One conversion site per direction across the client/sidecar boundary:** the Dart client converts Author→solver in `client/lib/domain/weight_profile.dart` before it calls `/segments/*`, and the sidecar's `SegmentRequest` continues to accept only a `theme` name or an already-solver `weights` map — it does **not** take the Author 0.0–5.0 scale. Server-side, `solver_profile_from_author` is that one site for any Python caller composing from a stored payload (hosted mode, tests, batch).

### 7.4 Multimodal routing (FR10–FR16) and its data dependency

Each travel **category** builds its own graph (`multimodal/`): cycling and hiking over the road/path network, paddling over the waterway network. The scorer is category-agnostic; graph construction and available edge attributes are category-specific. A **discipline** (issue #315) never changes the graph, the `network_type`, or legality — a `cycling` + `mountain` passage routes on cycling's `bike` graph with cycling's access rules, exactly as `mountain_biking` did before it stopped being a category; the discipline only selects the `WeightProfile`.
...
**v2.0 adds driving as a traversal mode** (PRD FR29, FR10). Driving legs to trailheads and put-ins are **routed by the same engine in driving mode**, not recorded as notes — because the last mile to the put-in is often the day's most harrowing part, and v1.0 flattened it into a text field.

**Measured 2026-08-31 — SPIKE-E; Q14 closed** *(amended)*. **Prior reading: "the cheapest new mode in the list (the OSM road graph is already built; the weights are near-trivial)", and unmeasured.** The engine half holds and the rest does not. Cold solves are **2.8–14.5 ms** over 722–12,702 edges, so driving costs the solver nothing — but `multimodal/modes.py` gives driving `network_type="drive"`, and **that is an OSMnx *download filter*, not a routing choice**: it excludes `highway=track` **and** `highway=service` before a way reaches the graph. Both non-`unclassified` trailhead access roads in SPIKE-E's set are exactly those classes, so the shipped configuration **routes to 265 m short of Gregory Canyon Trailhead and reports success** (the destination snaps to the nearest surviving node; the 3 km `OutsideGraphExtent` guard added by #154 never trips), and within 15 km driving distance of the Big Sandy trailhead it keeps **32.5 of 76.3 km — 42.6%** of the approach corridor. "The road graph is already built" was true and irrelevant: the graph is built to a filter that removes the last mile.

**Driving needs no weights of its own, and the ones it has do nothing.** `edge_cost` scales every penalty by `1 − directness`; driving ships `directness=0.95`, so `quiet=0.1`, `scenic=0.2` and `surface_paved=0.4` are applied at one twentieth strength, and sweeping `surface_paved` from −1 to +1 leaves the route byte-identical in three of four approaches. The mode is a near-shortest-path solve and should be an explicit one. What it actually wants is a **time** cost: `base_speed_kmh=60.0` flat reports 71.8 min for a 71.8 km dirt approach whose surface-aware estimate is **129.6 min (−44.6%)**, and over-reports by 19.7% where the drive is mostly paved.

**The bill Q14 closes with is four items, none of them a scorer** — so FR130's "adding a traversal mode requires only a new `WeightProfile` entry and its domain parameters" survives; the `TraversalMode` row simply needs a column `network_type` cannot express:

1. **A driving download filter that admits `track` and `service`** (osmnx `drive_service` plus `track`), rather than the `drive` network type.
2. **The way-tag list extended** — `4wd_only`, `motor_vehicle`, `motorcar`, `ford` — and a decision on node-tagged `barrier`, which `graph/regions.py` cannot carry in any form today (**#206**). See §7.9's note: folding node tags onto incident edges is graph construction's job and nothing does it, which leaves `_BARRIER_DEFAULTS` unreachable **for every mode**.
3. **`destination` removed from driving's `excluded_values`** in `routing/access.py`. It was carried over from cycling's row; for an access leg the trailhead *is* the destination, and `motor_vehicle=destination` marks a road the Character is entitled to drive (6 edges excluded on that ground in Boulder alone). It belongs in the advisory as an access note.
4. **A time model that reads the edge** rather than a flat mode speed (B7/FR16 owns it). Until it lands, FR29's "distance, **time**, and a cue sheet" is two of three.

Train, shuttle, and flight legs remain **authored notes**, not routes. That distinction is the whole point of the correction.

**Driving carries a vehicle-access advisory** (PRD FR29a, C13a). The signals: `surface=*` (gravel, dirt, unpaved, compacted, ground), `smoothness=*` (`very_bad` → `impassable`), `tracktype=grade1`–`grade5`, `4wd_only=yes`, `highway=track`, and `motor_vehicle=*`/`access=*`.

**Measured 2026-08-31 — SPIKE-E** *(amended)*. **Prior reading: "OSM supports it better than expected", and `smoothness` "the strongest single indicator".** Half survives, with the denominator corrected. Over four regions, **`4wd_only` is tagged on 0 of 12,069 ways / 2,419 km** — the requirement names it first and the data does not have it, so it is an opportunistic read and no coverage statement may count it. **`smoothness` is `absent` at network scale in every region (0.3–4.3%) and covers 100% of the last 3.4 km of the worst approach in the set** — it is the strongest indicator *on the roads that need one*, not on the network, and reading its network figure would have been the wrong number to act on. `surface` carries the feature (79.9 / 24.8 / 22.6 / 33.6% of ways at network scale; 97.0 / 44.8 / 82.6 / 78.5% on-route). **Coverage does not narrow monotonically toward the approach**: on a remote forest road the route is *better* surveyed than its network (85.1% vs 62.8% of km) because the people who tag it are the people who drove it, while a put-in road reached by a `highway=service` stub is **0.6% surveyed over its last 5 km**. Both are the same mechanism, and it is why coverage is computed **per last mile as well as per leg**.
...
**Three implementation constraints SPIKE-E measured rather than assumed.** **(a) The honesty floor is the `read` band (≥70% of the leg's kilometres carrying a condition tag), not the `opportunistic` one.** At 20% a real 35%-surveyed gravel approach reports "no signal on this route exceeds 2WD"; a seeded thinning model puts that confidently-wrong outcome at **18.8% of trials against a pre-declared 10% ceiling**, falling to **0.0%** at 70%. Below the floor the only honest state is *unsurveyed*, and its phrasing shares no claim with the clear one. **(b) Sections merge; they are never per-edge.** Condition tagging is fragmented along one continuous road, and a per-edge advisory emits **65 sections** for a single 42.6 km dirt approach. Runs merge at the same requirement across **unsurveyed** gaps up to 500 m and never across a stretch tagged clear. **(c) `motor_vehicle` is an access note, not a capability flag** — it says who may drive, not what they need to drive in, and an Author cannot act on it by borrowing a better vehicle.

### 7.5 Elevation — void handling

Validated by the POC (FR85–FR91, SPIKE-18); **amended 2026-09-24 by #473 (D68)** to separate a *gap* in a raster that opened from the *absence* of any raster:

- value present → use it
- **gap** — `nodata` sentinel; **NaN nodata, checked explicitly via `isnan`** (`value == ds.nodata` misses NaN under IEEE 754 — a real defect found and fixed in the POC); `±inf`; coordinate outside the open raster's bounds → **linearly interpolated** from the nearest finite samples: by along-track distance for an ordered route (`ElevationSampler.sample` / `void.interpolate_voids`; an end gap holds its one finite neighbour), by graph adjacency during enrichment (a void node takes the mean of its known neighbours, spreading hop by hop). `0.0` only when there is no finite sample anywhere to interpolate from.
- **absence** — raster missing or unreadable, or no source resolved the bbox → **absent**: `sampler_for` returns `None`, a degraded sampler reads NaN and profiles to `{}`, enrichment writes no `elevation` / `elev_gain`. Never a flat `0.0` that reads as measured.

*Prior reading (v1.0 → 2026-09-24): all five cases → `0.0`.* Every void **logged once per raster path**, never per coordinate. Each lookup reads a **single-pixel window**, bounds-checked before the read; datasets opened lazily and held for the process lifetime.
...
GET    /tiles/{z}/{x}/{y}               # z/x/y range-validated before upstream work (FR93); region cache → home archive → upstream read-through (#154)
...
GET /health →
{
  ...
  "capabilities": {
    ...
    "elevation": {"ready": true, "reason": "opentopography",      # ← source, process-wide (#148)
                  "regions": {                                     # ← per region, like routing
                    "7aee30aebeddc034": {"ready": false, "reason": "waiting for this area's routing data",
                                         "progress": 0.0},
                    "a51f9c2e0d8b7461": {"ready": true}}}
  }
}
...
- **Layer readiness is per layer, not one flag** (PRD N2). A plugin dataset may be large or remote, so `capabilities.layers` carries per-layer state: built-in OSM layers go ready while a plugin layer loads, the picker shows that layer as loading rather than blocking the workspace, and **one layer failing never blocks the others** — the failure names the layer and the reason. This is B1's rule applied one level down: any long operation standing between the Author and their work gets the same treatment, and a layer set is exactly such an operation once plugins exist.
  **Measured 2026-08-28 (SPIKE-D) at 3 of 8 of this contract's clauses; landed since — N2 (#2) and #400.** SPIKE-D found `capabilities.layers.per_layer` was the constant `{layer: "ready" for layer in sorted(LAYERS)}` with no seam at which a layer could report `loading` or `failed:licence_missing`, and `GET /candidates` wrapping one provider call in one `try` so a single failing layer returned **422 for the whole request** — the exact inverse of this rule. **The cause was a divergence from §14.2, not a gap in it**, and N2 closed it by reconciling `providers.py` with that contract: `curation/registry.py` registers each layer individually so `per_layer` is a real map, refuses a layer whose licence is absent or unsatisfiable **at registration** (`failed:licence_unsatisfiable`, never queried), reports `per_layer` / `per_layer_detail` on `/health`, and `fetch_candidates_all` returns `(candidates, per_layer_error)` so `/candidates` answers **200 with what it did get** plus `layers_served` / `layers_unavailable`. #400 (2026-09-15) gave the client the matching M13 state — **layers-partially-served** — so the Layers tab names each missing layer and its reason and a partial extraction never reads as an empty area. The prototype that demonstrated the shape lives in `spikes/SPIKE-D/`.
- **Routing readiness is per region, not one flag** (D57, issue #154). D41's trip bbox is the Author's own authoring extent, so "routing" is never one process-wide state any more — it's keyed by the region id `POST /regions` returned, empty until an Author has drawn a bbox. **Elevation is per region too since #148 (D69)**: the top-level `ready`/`reason` states the process's *source* (OpenTopography direct with a key, the QA proxy, or the local cache only — the last with a finished sentence saying so), and `elevation.regions[key]` states what each region got — waiting on its graph or fetching (with `progress`, never an `eta_s`: a network fetch has no honest estimate, #397), `ready`, or a finished sentence saying why that bbox has none. Absent, never flat (FR88/#473).
...
### 8.6 Request threads, outbound calls, and deadlines *[NEW 2026-09-22 — D66, after #466 and #488]*

**Every endpoint in `plotlines-service` is a sync `def`, and FastAPI runs every sync `def` on one process-wide thread pool** (anyio's default capacity limiter, 40 threads). `/health`, `/layers`, `/tiles/{z}/{x}/{y}` — the endpoints the client polls, paints from, and gates the whole app on — answer from the same 40 threads as anything that goes to the network. That is the whole mechanism behind #488: a `/health` poll every 2 s, each one fetching `MIRROR_STATE.json` inline, each fetch's DNS lookup stalling on WSL, and the pool filling with stuck `/health` threads until `/layers` and `/tiles` — near-instant local reads — could not get a thread at all. It was the second time this class of bug took the sidecar down (#466 was the first, on the same platform and the same DNS leg), and the review that followed found five more live instances and the client half of the same pattern (#490–#496).

Three facts the code has to be written against, none of which a docstring holds on its own:

1. **No library's timeout covers name resolution.** `urllib`, `requests`, `http.client` and `httpx`'s default sync transport all bottom out in `socket.create_connection`, whose `getaddrinfo` call is a blocking OS call with no timeout parameter anywhere in the stdlib. A `timeout=` bounds the socket *once it exists*. A stalled resolver — the WSL-DNS class, a captive portal, a dead LAN DNS on a Pi — hangs the calling thread past any timeout it was given. **The only reliable bound on such a call is a deadline on the caller's side: stop *waiting* on it, accepting that the underlying thread may outlive the deadline.** No library swap closes this gap for a synchronous call.
2. **A lock held across an outbound call imports that call's unboundedness into every waiter.** `OSM_SETTINGS_LOCK` and `_NOMINATIM_LOCK` each exist for a good reason — process-global `ox.settings`, a 1 rps policy — and each was implemented by holding the lock for the whole round trip. The lock then serialises not the mutation but the *wait*, and every thread queued on it is a shared-pool thread that `/health` cannot have. Hold a lock around bookkeeping; never around a socket.
3. **A client-side timeout does not free a server thread.** A Dart `.timeout()` abandons the future; the sidecar thread keeps running. `_healthPollInFlight` stops the *client* from stacking polls, and the very next tick after a timeout fires a fresh request against a pool that still holds the last one. Client timeouts are necessary (the Author gets an honest state, §8.3 rule 3) and not sufficient (the server still needs its own deadline). Both halves are owed on every call that can block.

**The contract, in five rules:**

- **A request thread on the shared pool makes no outbound call and takes no lock whose holder might be making one.** An endpoint that needs the network submits the call to a **named, dedicated pool** and waits on it with `future.result(timeout=DEADLINE)`. On expiry it returns the honest degraded answer for that one capability — `stale` with a reason, `failed:<reason>`, a finished sentence in a 422/503 — never a 500, never a hang. `_mirror_capability` / `Readiness._mirror_state_pool` (#488) is the reference shape; `/candidates` (#490), `/geocode` (#493) and `/clip` (#494) are owed the same.
- **Every deadline is a named constant with a margin over the call's own socket timeout**, so the common fast case never trips it and the stuck case always does (`_MIRROR_STATE_FETCH_TIMEOUT_S = 8.0` over `load_mirror_state`'s 5.0). A deadline that only bounds the happy path is not a deadline.
- **A dedicated pool is small and owns its queue.** One worker is right for a poll (a second poll queues behind the first rather than spawning a second stuck thread anywhere that matters); a bounded few is right for a job. A job pool with one worker and jobs that can hang needs **per-phase deadlines inside the job** too — `RegionState.build`'s four sequential network phases on `REGION_BUILD_CONCURRENCY = 1` are the open instance (#492): a hang in tiles after `graph_state.succeed` leaves the region `ready` and every later region `pending` forever, which no requeue path reaches.
- **A failure on a transport path is transient by default.** A stalled resolver or a 502 is a fact about *this request*, not about the layer, the region, or the session: it is reported for this request, retried on the next under a cooldown-and-cap (#247's shape), and never latched into a state only a restart clears. The candidate registry's sticky `FAILED` after one `CandidateFetchUnavailable` (#491) is the counter-example; plugin `load_state()` failures at registration are the case that *may* stay sticky, because a missing licence is a fact about the layer.
- **Long CPU work has the same rule as long network work.** A ~100 s pyosmium clip inline on a sync handler occupies a shared-pool thread exactly as a hung fetch does, and "single process" (`--workers 1`) bounds nothing — anyio still hands out 40 threads. Concurrency is a decision in code: a semaphore with 503 + `Retry-After`, or a 1-worker pool with D25's 202-and-poll (#494).

**What this is not.** It is not a move to `async def`: an async handler that calls a blocking library blocks the event loop itself, which is strictly worse, and every outbound library this service uses is synchronous. It is not a call to switch HTTP libraries — see fact 1. And it is not a per-endpoint thread pool: the point is that the *shared* pool never blocks, and one dedicated pool per outbound *concern* (mirror state, Overpass, Nominatim, region build) is enough to guarantee that.

**Enforcement** is #497: a service-suite test that blocks `socket.getaddrinfo` and each library entry point, fires one of every outbound-touching endpoint concurrently, and asserts `/health`, `/layers` and `/tiles` still answer in under a second while every stuck call gives up by its stated deadline; plus a static check that no `@app.*` handler body reaches an outbound call directly. Until it lands, this section is held by review — the same window punch-list §7 / addendum P6 name as the one where an endpoint list grows under pressure.
...
| **OSM Overpass** (via OSMnx) | Fallback when no mirror clip is cached yet; Phase 5's capped live-refresh affordance (#285, not yet built) | Long | OSM / ODbL |
...
**v2.0's OSM load is now met with a mirror, not a bigger Overpass budget.** The prior reading here — "candidate extraction pulls far more tag classes over a trip bbox than graph building, so it is a heavier query" — was corrected by SPIKE-D (2026-08-28) and stands measured backwards: a candidate pull is ×0.43 the time and ×0.78 the bytes of a graph build over the same bbox. What actually forced the acquisition redesign was throttling variance and single-endpoint dependence, not query weight (A23). The mitigation is no longer "read from a local extract wherever a region is used repeatedly" as a best-effort pattern — it is the default transport: both `ensure_graph` and `OsmLayerProvider.fetch` resolve from one shared mirror-side clip before either ever calls Overpass (D63, Phase 3 #272). `docs/Plotlines_OSM_Acquisition_Review.md` is the full record — its own §12 answers the six questions this redesign turned on; this section does not restate them.
...
**Phase 1 is wired (#148, D69).** Every region build resolves its bbox through `phase1_resolver_for_layout` after the graph is ready, on the elevation build-phase pool behind its deadline (§8.6): the bbox-scoped local cache first, then OpenTopography directly **only when `PLOTLINES_OPENTOPOGRAPHY_API_KEY` is set** in the environment the sidecar inherits. FR87 is enforced by `elevation/keys.py` before the wire (the free tier's 50 *new bboxes* per rolling 24 h; the commercial-posture refusal), with the call ledger at the cache root. With no key the cache is still read, so a previously fetched or shipped (FR90) bbox has elevation offline. A resolved raster enriches the region graph — node `elevation`, edge `elev_gain` and osmnx's `grade`/`grade_abs`, which FR2's peaks weight reads and which before #148 only the committed fixture carried — on a copy swapped in whole, so a solve running since routing went ready never sees a half-annotated graph. `--elevation-upstream` (the Pi5 QA proxy) replaces the direct provider entirely, never falls back to it.
...
**One policy, every payload, not one mechanism plus two exceptions *[issue #269]*.** The dynamic check above covers everything that is a *layer*. Elevation's CC BY and the basemap's ODbL are not layers — they always ship with the home region regardless of what plugins are loaded — so they are asserted as a **second, static half** of the same gate rather than being left to the dynamic enumeration to somehow cover. The routing graph is the same shape: built from OSM under ODbL, always present (routing is one of the three capability gates that always starts, §8.3), and not a `LayerProvider` either — so it is the **third** static obligation, not a special case bolted onto "elevation and basemap." Read this the way §8.1 reads tile/elevation/candidate caching: **same policy, N payloads, not N designs** — here N is three static plus however many layers are loaded, and adding a fourth static obligation (Phase 3's mirrored `.osm.pbf` extract path, for instance) means adding one entry, not rethinking the gate.
...
|---|---|---|
| **Data attribution & licenses** | **Required, all platforms** | CC BY (elevation, weather), ODbL (basemap, **the routing graph** — its own credit line since #269, not inherited from the basemap by accident, OSM), **and every loaded plugin layer's own terms (§12.2)** — enumerated dynamically, not hardcoded. Build failure if absent. |
...
**A provider's `taxonomy` is consulted by its own `fetch_candidates`, never merged into a shared table.** N5 (#6, 2026-08-28) shipped `notability.score_with_taxonomy(features, taxonomy, live_layers)` — the parameterised body `score_notability` now delegates to, closed over the built-in `TAXONOMY` for OSM and called directly by a plugin's `fetch_candidates` against its **own** declared instance. A plugin scores its own types and returns finished `Candidate`s; its rows never get folded into `curation/taxonomy.py`'s table, which is exactly the core-code edit §14.4 forbids. This is what makes the licence/taxonomy/load-state split above real rather than aspirational — a plugin author owns a `TypeTaxonomy` value, not a pull request against core.
...
**The shipped `providers.py` now implements this contract — N2 (#2), after SPIKE-H.** Before N2, `core/plotlines_core/curation/providers.py` collapsed `licence -> LayerLicence` to `licence: str`, dropped `taxonomy` and `load_state()`, and replaced `fetch_candidates(bbox) -> list[Candidate]` with a multi-layer `fetch(bbox, layers) -> list[RawFeature]` whose bare-list return left per-layer state and per-layer failure nowhere to live — the direct cause of one failing layer returning 422 for the whole extraction (§8.3). N2 landed the four-member `LayerProvider` Protocol above (`licence`, `taxonomy`, `fetch_candidates(bbox)`, `load_state()`), with **`SharedOsmFetch`** reconciling the one bend SPIKE-H recorded — six built-in layers, one Overpass (now one clip) call — and `curation/registry.py` enforcing D45's licence refusal at registration. The legacy `OsmLayerProvider.fetch(bbox, layers)` remains for backward compatibility; new work is written against `LayerProvider`. `spikes/SPIKE-H/` still carries the prototype that demonstrated the reconciled shape against real upstreams.

**The four graph-data Protocols now exist, as interfaces only** — `core/plotlines_core/providers/graph_data.py` (FR84, issue #147). Core ships **zero implementations** of them: the OSM defaults reach the graph through `graph/loader.py`, not through a registered provider. `NodeDataProvider` and `ShapeDataProvider` return `RawFeature` — the same raw shape `LayerProvider` extraction produces, rather than a second one — and `plotlines_core.providers` re-exports `LayerProvider` so both directions of the data-input contract read as one page. `EdgeDataProvider` annotates and never restructures (P6), and what it adds is **advisory**: mode-legality (FR128) stays the constraint category, decided in `routing/access.py`.

**SPIKE-17 (#176) ran 2026-09-01 against those Protocols and the contract holds — with two revisions this section owes.** Two deliberately unlike real providers (511 WI's WZDx 4.2 work-zone feed, 4,424 events; NWS active alerts) annotated two real graph builds — 42,450 and 138,979 edges through the shipped `ensure_graph` — **with no core change**. Annotation costs **0.67 s / 2.25 s against builds of 17–91 s**, so it needs no readiness gate of its own and rides the graph's own startup pass. P6 was asserted on the bytes: **0 of 18 core-consumed edge keys mutated**, with annotations confined to an `advisory:` namespace nothing in core reads. The normalisation-proxy question answered **no server, and therefore no P3 design event** — a per-source adapter is ~30 lines while the shared map-matcher is the real work and is device-side regardless. See `spikes/SPIKE-17/results/RESULTS.md`.

**The two revisions, both from bends a real provider hit immediately.** First: **the annotation Protocols must declare `licence -> LayerLicence` and `load_state() -> LayerLoadState`**, as `LayerProvider` does, with D45's registration-time refusal in front of all four. This is measured, not hypothetical — of two conformant WZDx feeds, **511 NY declares no licence at all**, so under §12.2 it must not load and this section as written has nowhere to put that refusal; and without `load_state()` an unreachable source is indistinguishable from "no work zones here", which for an advisory reads as all-clear. Related: a feed declares its terms *inside the document*, so `licence` cannot stay a pre-fetch property for a feed-shaped source. Second: **the advisory namespace rule belongs in the contract**, where an implementer reads it — `annotate_edges` returning the graph invites writing to any key on it, and that is the one-line change that would quietly move a DOT feed from FR14/FR29a's advisory category into FR128's constraint category. A third, smaller: `annotate_edges -> Graph` has nowhere to report what it did, and an Author-facing "12 of 14 work zones located" needs the match rate, the unplaceable events, and the out-of-bbox count. **Reconciling these is Leg 7's annotation story, not this document's** — the SPIKE-H precedent: the spike hands the next story its target shape and `spikes/SPIKE-17/` carries a working implementation of all three against real upstreams.

**Two findings that constrain the annotation half regardless of the contract.** A source's **publication shape matters more than its declared spec version**: 511 NY publishes every event as a single point, and degrading the same Wisconsin events to points recovers **12.9%** of the line match's edges while putting **46.5%** of its claims on the wrong road — so a registry cannot judge a source by its version string, and geometry kind is a quality signal a contributor should declare. And **`EdgeDataProvider` is the wrong seam for area-scale sources**: NWS's county polygons, once resolved, annotated **100%** of a region's edges. `ShapeDataProvider` already exists for that, and this is what it is for.
...
**The seam shipped 2026-09-01 (FR84, issue #147), and its contract shape stays open as FR84 requires.** `client/lib/data/plugins/output_integration.dart` and `plugin_registry.dart` declare the shape; **no destination is named in code and no vendor SDK is depended on** — Garmin / Coros / Wahoo / RideWithGPS and the destination list stay open. (`SPIKE-17` ran on 2026-09-01 and settled the *input* direction's packaging story: reuse `curation/plugins.py`'s entry-point mechanism, one group per direction. The output direction's own distribution question — pub.dev, Q8's Dart half — is untouched by it.) `client/lib/data/reveal_view.dart` supplies the `RevealView` the signature above always named: it is the object that crosses the boundary, and it carries the three obligations that attach to the same event — reveal (`rolesFor`, hazards always released), attribution (`attribution`, seeded with the always-owed elevation and basemap credits), and encoding (a `TripPayloadWriter` over the core's writers, so D58's "the integration does not carry its own" is structural). `PluginRegistry` is the gate: unique ids, a push refused outright when the view carries no credits (`MissingAttributionError`, mirroring the core's `assert_attribution_complete`), format negotiated before the integration is called, and the `SecureStore` owned by the registry and passed in for the duration of a call rather than held by the integration.

**Three deltas from the sketch above, all deliberate.** `authenticate(SecureStore store)` takes the store rather than reaching for one, so an integration never owns credential storage. `pushTrip` returns a `PushOutcome` rather than `void`, carrying a `ReasonCode` **from the bounded phrase table** — a vendor error string interpolated into a user-visible message is exactly the unbounded slot FR145 / D57 forbid, so the typed outcome is a requirement rather than a convenience. And an integration declares `accepts: Set<ExportFormat>`, which is what lets the registry refuse an impossible push before any content leaves the device.
...
- **★ Request-thread contract tests** (§8.6, #497): with `socket.getaddrinfo` and every outbound library entry point blocked on an `Event`, one of each network-touching endpoint in flight, `/health`, `/layers` and `/tiles` still answer in under a second, every stuck call gives up by its named deadline with an honest body, and a second `POST /regions` is not left `pending` behind a wedged build. `service/tests/test_health_mirror_stuck_fetch.py` (#488) is the seed. **This is D66's regression test**, and the reason it is a test rather than a rule is that every instance the post-#488 review found (#490–#496) sat under a docstring explaining why that call was safe.
...
| **A23** ★ | **Restated 2026-09-17 — SPIKE-I / #287, closing the "still unmeasured" clause.** *(History: mechanism corrected and severity raised 2026-08-28 — SPIKE-D. A candidate pull measured ×0.43 the time / ×0.78 the bytes of a graph build over the same bbox, so the exposure was never query weight but **throttling variance** — the identical 176 km² query took 8.8 s and 185.9 s an hour apart — plus single-endpoint dependence; during SPIKE-D one uncached pull ran past 30 minutes and got `overpass-api.de` to refuse TCP from that host for the rest of the session.)* **That risk no longer describes the primary path.** Phase 3 (#272, closed 2026-09-17) put both extraction consumers — `ensure_graph` (`graph/pbf_source.py`) and `OsmLayerProvider.fetch` (`curation/providers.py`) — behind **one shared mirror-side clip** (`graph/extract_fetch.py:find_reusable_extract`); public Overpass is now reached only as a fallback when no clip is cached, and by the not-yet-built Phase 5.2 capped live-refresh affordance (#285). The clause this row used to close on — **"local extracts remain the only mitigation that removes the variance, still unmeasured"** — is now measured, and the finding is not the happy one: SPIKE-I (#265, 2026-09-13) found the clip itself costs **432–553 s wall / 1.9–7.2 GB peak RSS, O(region extract) not O(bbox)**, and the two-extract merge path **failed 0/4 times** (`MergeInputReader` buffering raw extracts, +3.9 GB in 5 s, OOM). Both root causes were fixed the same day — **#375** pre-cuts pinned extracts to the served corridor at pull time so the per-request scan is already small, **#376** merges the few-MB *clipped outputs* rather than the raw extracts — and **the fix was re-measured on the live Pi 2026-09-18 (#402, PR #438)**: the same Asheville cell went from **617 s** on the full-state pin to **101.65 s / 101.73 s** on the `--precut-wnc-corridor` pin — an 83% reduction that tracks the 6.4× drop in pinned-extract size almost exactly (O(pinned extract) confirmed), and **still ~1.7× over SPIKE-I's pre-registered ≤60 s outer band**; that residual is **#439**. The same pass fixed rectangle-based extract over-selection (each extract's real `.poly` boundary now narrows `select_covering_extracts`), cut the clip's peak RSS 2,842 → 1,770 MB with a disk-backed location index, added an opt-in clip cache, and found-and-fixed a live defect where precut output carried no header box, so an out-of-corridor bbox paid a full scan before 404-ing (now 2 s). | ~~HIGH~~ **MEDIUM** | The variance/single-endpoint risk is retired for the default path by construction, not by tuning — the dependency is now a Plotlines-operated mirror, not a volunteer public instance, and §11.3's "we become the availability" risk that trade creates is tracked separately with a shipped mitigation (`MIRROR_STATE.json` + mirror age on `/health`, §6.6/Q2). What is left open: (1) **#439** — `/clip` at ~102 s on the live Pi is measured but still over SPIKE-I's band; closing that needs a smaller pinned extract or the spatial index #402 evaluated and declined, and this row stays Medium until it lands; (2) the residual Overpass fallback and Phase 5.2's capped affordance still carry **A23a**'s unpatched library defects, below. *(Rectangle-based over-selection, formerly item 3 here, was fixed by #402/PR #438 via each extract's `.poly` boundary.)* See `spikes/SPIKE-I/results/RESULTS.md` §3, ARCH **D63**. |
| **A23a** ★ | **Revisited 2026-09-17 — #287.** *(Original finding, SPIKE-D 2026-08-28, unchanged as a code fact: two unbounded waits in `osmnx` are inherited by the product. `_get_overpass_pause` does not parse the status page — it takes line index 4 and branches on its first word; against an instance reporting `Rate limit: 0` that line is the header `"Currently running queries (…)"`, read as "the server is busy with my query", so it sleeps 5 s and **calls itself, forever** — observed 20 minutes in `hrtimer_nanosleep`, no query ever sent. Separately, `_overpass_request`'s 429/504 handler sleeps 55 s and recurses with **no attempt limit**.)* **Neither defect was patched.** Phase 0's fourteen mitigations (#254) bounded the *application*-level retry loop around osmnx — a settle window and supersede-in-flight (#246), a cooldown-and-cap between a settled failure and the next requeue (#247), `overpass_rate_limit` kept on through failover (#245) — but no `Deadline` wrapper or endpoint-pacing sits inside the osmnx call itself, and the single-endpoint candidate-path posture was **recorded as accepted, not fixed** (#250). What changed is **exposure, not the defect**: Phase 3's transport swap means most requests now resolve from the mirror clip and never call `osmnx.features_from_bbox` / `ensure_graph`'s Overpass branch at all, so A23a fires only on (a) the shrinking fallback window before a bbox's clip is cached, and (b) whatever Phase 5.2 (#285, not yet built) ships for the capped live-refresh affordance — an *area* cap does not bound the *time* a stuck `_get_overpass_pause` call can spend, so #285 inherits this defect unless it wraps the call in a deadline explicitly. | Medium | Unchanged in kind from SPIKE-D: wrap the call in a hard deadline (`spikes/SPIKE-D/common.py:Deadline` is still the pattern), detect an unparseable status page rather than trusting the library's limiter, own the endpoint list. **Owed to #285** as an explicit acceptance clause, not assumed inherited from Phase 0. **Sharpened 2026-09-22 (A30, #490):** both unbounded waits now run *under `OSM_SETTINGS_LOCK`*, so they are inherited not only by the calling thread but by every `/candidates` request queued on that lock — and those are shared-pool threads. The deadline wrapper is no longer only #285's concern; §8.6 makes it the rule for every outbound call. |
...
| **A30** ★ | **A blocking call on a shared request thread takes the sidecar down** *(added 2026-09-22 — #466, #488, and the review that followed)*. Every service endpoint is a sync `def` on one 40-thread pool; no library timeout covers DNS; a lock held across a socket makes every waiter as unbounded as the holder; a client timeout does not free the server thread. Realised twice (#466, #488) and found live in five more places and the client (#490 `OSM_SETTINGS_LOCK` across the Overpass round trip; #491 six-call outage and sticky layer `FAILED`; #492 no deadline on the one-worker build pool; #493 `/geocode` under `_NOMINATIM_LOCK`; #494 `/clip` unbounded on the mirror; #495 `elevation_proxy`; #496 no client timeouts). The failure looks like the app hanging — a spinner on the Layers step, "No basemap tiles here", `pending` forever — not like a network error, so it is misdiagnosed as the feature that was being tested (#488 surfaced while testing #453) | **HIGH** until #490/#492/#493/#496 land, then Medium | §8.6's five rules and D66; the #488 shape (dedicated pool + `future.result(timeout=)` + honest degraded answer) as the reference implementation; transient-by-default failures with #247's cooldown-and-cap; the #497 gate so the next instance fails CI rather than a live Pi session. **The instinct to fix this with a library swap or `async def` is wrong both times** — see §8.6 "What this is not". |
...
| **D37** | **Areas are first-class geometry across payload, triggers, export, and local schema** *(breaking — B3)* | PRD FR108. A rest day on a main street or in a historic district is inexpressible as a point, and polygon *entry* is a different trigger from point proximity. **`ShapeDataProvider` already existed** (§14.2), so the provider layer needed no change — the architecture was more right than v1.0's requirements. **That is a claim about the protocol, not about candidates or anchors** — `Candidate`/`RawFeature` had no field to carry a polygon until N5 (#6, 2026-08-28) added `area_m2` and an exterior ring so a scored area feature is not indistinguishable from a pin (`notability.py`, SPIKE-H §3) — and that ring stopped at the Python dataclass: `/candidates` never emitted it, and a line (SPIKE-H's 9,961-vertex byway) still collapsed to a centroid. #403 (2026-09-21) made the path whole: `geometry` is a kind-tagged `Shape` (`polygon` \| `line`) that `feature_from_geometry` fills for both kinds (a line's `coord` is a point *on* the path, not its off-road centroid), `colocate.py` clusters a line at the midpoint of its in-bbox run rather than through the polygon clip, the candidate cache and `/candidates` carry it as RFC 7946 `Polygon`/`LineString`, and the client's `Candidate.geometry` reads it typed. `promote.dart::areaFromCandidate` is the adoption D36's copy-not-reference rule calls for — a polygon becomes `Anchor.area` with `source: imported`, a line yields none (an anchor is a point or a polygon; a byway is passage-shaped) — and O3 (#10) owns wiring a candidate promotion onto it. The interface surviving a contradicted requirement did not, by itself, make an area candidate real | Point-plus-radius as an area approximation (a historic district is not a circle, and the approximation is worst exactly where the boundary matters); areas as presentation-only overlays (then they cannot trigger, which is half the point) |
...
| **D60** | **Plotlines' trip payload is a Collective Database, not a Derivative one: the OSM-derived anchor layer (coordinate, name, category, tags — via `AnchorProvenance.layer`, shipped 2026-08-24) stays structurally separable from Plotlines/Author-authored content, which is what keeps ODbL share-alike from reaching Plotlines' own tables. Design posture: produced works and mirror-side clips only — hosted web and the anonymous reading view serve rendered output, never a raw payload dump, to an unauthenticated party; any authenticated API surface exposing `anchors[].provenance` stays Author-or-invited-only** *(new — `docs/Plotlines_Licensing_Position.md`, issue #253, 2026-09-04; closes addendum L2/L3/Q4-A)* | ODbL's share-alike and offer-of-database conditions (§4.4/§4.5) attach on *Public Use*, which includes serving over a network — not only on handing over a file, so hosted web and any future trip-sharing surface implicate it, not just the mirror's own bbox-clip redistribution (which is unrelated literal OSM redistribution, already governed by L4/L6). Checking L3's question against the tree rather than against the addendum's own `Trip.days[].segments[].nodes[]` reading found the per-feature discriminator it asked for already shipped, as `AnchorProvenance` on the *current* promoted-place model (`Trip.anchors`, D36) — the addendum was written against the superseded day-scoped `Node` shape. Attribution is owed on every surface regardless and is already mechanical (D45) — this decision is what keeps the harder *share-alike* question from ever needing to be litigated per-trip | Treating the trip payload as one merged Derivative Database (forces share-alike onto Author-authored notes, roles, and arc the moment any OSM-derived anchor is in the trip — the worst-case reading, and unnecessary once the layers are kept separable); resolving this only at Phase 4 as the addendum's reviewed document originally sequenced it (share-alike is already implicated by cloning and anon reading, both decided pre-Phase-4 surfaces, not gated on hosted web shipping); building a new source-discriminator field (the shipped `AnchorProvenance` already is one — building a second would duplicate it); leaving the offer-the-database mechanism unbuilt with no plan (filed as **#301** rather than left implicit) |
| **D61** | **A discipline is a variant under a travel-mode category, not a category of its own (Model B). The `travel_mode` enum is the five traversal categories plus `transit`; `mountain_biking` / `packrafting` / `riverboarding` are removed and become the `mountain` / `packraft` / `riverboard` disciplines; `segment.discipline` is a new optional field. Categories are declared at trip init; a discipline is per-passage. `multimodal/legacy.py` migrates a pre-#315 payload before validation** *(new — issue #315, 2026-09-09)* | The three removed values already shared another category's graph, `network_type` and legality and differed only in weights — "Model B expressed as a Model A row" (#315). Making a discipline its own `travel_mode` (Model A) would multiply a value that keys `resolve_default_layers`, `network_type_for`, the payload enum and `declaredModes`, for a distinction the scorer already expresses as a `WeightProfile`. The removed values stay resolvable as a `theme` string and via `access_mode_for` so an un-migrated payload still routes and weights correctly. Layer defaults key off the *category*, never the discipline (owner's call, #315) — a packraft trip and a kayak trip get the same default layers for now. `grades_difficulty` is recorded per discipline for future B9/FR14b work and nothing reads it (SPIKE-C) | Model A — a discipline is a `travel_mode` row (multiplies four keyed lists for a weights-only distinction); keeping `mountain_biking` etc. as deprecated aliases in the enum (leaves the inconsistency #315 exists to resolve — two spellings of one route character); per-discipline layer defaults (owner scoped out; revisit if put-in / portage layers need it); building difficulty grading now (SPIKE-C: OSM land grading is too thin to aggregate; plugin territory) |
| **D62** | **Offline bbox-edit posture: OSM acquisition Q1-C (mirror-side clip to the trip bbox) stays as shipped; Q1's D fallback (client-pulled region extract, re-clipped locally offline) is rejected on measurement, not merely left un-triggered** *(new — SPIKE-I / issues #265, #278, 2026-09-17; closes the open fallback clause in `Plotlines_OSM_Acquisition_Review.md` §6.7/§12-Q1, adds §6.7a)* | PRD FR120, FR121; D41/D57's offline-first posture. SPIKE-I measured both halves of the question §6.7 deferred. **D's precondition is real**: an Author editing the trip bbox with no connectivity is servable only 38–48% of the time against a pre-registered ≥90% band (100% of shrinks, but 13–34% of nudges and 1–10% of grows, because the held graph covers only the bbox plus osmnx's own ~7%-of-span buffer) — so the offline edit that would force D is not hypothetical. **But D's mechanism doesn't survive contact**: it means running the mirror's own clip — measured at 432–553 s wall and 1.9–7.2 GB peak RSS **on a Raspberry Pi 5**, hardware with more headroom than most client devices, with no predictable relationship between an extract's size and its memory cost — on the client, offline, in the field. That reopens **L1**'s native pyosmium dependency and un-freezes **#266**'s freeze matrix on every platform, for an operation with no way to size its cost in advance. The addendum's "D is a configuration decision later, not a rebuild" is corrected: true of storing pinned extracts as static files (unchanged), false of what a client does with one offline (the half nobody had measured). C's offline experience is specified instead: a nudge or grow past the held buffer disables the newly-uncovered area's capabilities with a stated reason — already shipped, via #274/#275's `failed:<reason>` capability reporting; a shrink's improvement (truncate the held graph and serve it provisionally, 0.6–1.6% node divergence from a fresh build, disclosed rather than silent) is the **decided target**, filed as **#432**, not yet built — until it lands a shrink reports the same honest not-ready as any other offline edit. FR120's no-lost-anchor guarantee holds unconditionally in every case, built or not | Taking D as the fallback now that its trigger fired (reintroduces the client-side native dependency and a multi-minute, multi-gigabyte operation into precisely the offline path it exists to rescue); treating B9's low servable percentage as itself sufficient reason to take D (conflates "the precondition fired" with "the fallback is viable" — SPIKE-I measured the two independently and here they point opposite ways); loosening the pre-registered ≥90% servability band after seeing 38–48% (the band was fixed in `spikes/SPIKE-I/HARNESS.md` before the run for exactly this reason); silently discarding provisional-graph status and presenting a shrunk offline graph as final (the 0.6–1.6% divergence is small but measured, and FR121's honesty requirement does not have a de-minimis exception) |
| **D63** | **Bulk OSM acquisition for the routing graph and curation candidates is a mirrored Geofabrik extract, clipped server-side per trip bbox — never a bulk pull against a public Overpass instance. Small, capped, user-initiated Overpass use is retained (a "refresh from live OSM" affordance, Q5-A) and the policy is mechanical, not discretionary: the default endpoint list cannot regrow (#251) and every acquisition path resolves through one shared clip (`graph/extract_fetch.py:find_reusable_extract`) before falling back to Overpass at all** *(new — `Plotlines_OSM_Acquisition_Review.md` / its licensing addendum, accepted 2026-09-03, executed through Phases 0–3 #254/#264/#268/#272, 2026-09-17; issue #287, closing review checklist item 26)* | The review opened on A23/A23a: a public, volunteer-operated Overpass instance is a dependency Plotlines does not control, with ×21 run-to-run throttling variance and a demonstrated capacity to block a whole host mid-session (SPIKE-D). §11 priced what mirroring trades away — becoming the availability (§11.3), a native pyosmium dependency (§11.2, resolved PARITY by SPIKE-J), a Derivative-Database redistribution question (§11.6, resolved by D60) — and §12 answered the six open questions the plan turned on: Q1-C (mirror-side clip, not a client-held region extract), Q2-B (monthly pin, named owner, monitored), Q4-A (the licensing position, D60), Q5-A (keep a small capped interactive affordance rather than eliminate public use entirely), Q6-D+C (clip server-side, bulk on zero-egress storage — SPIKE-I measured the egress win at ~325×, #287). The alternative the review rejected throughout — treating this as a one-line endpoint swap — is exactly what **A23a** shows cannot work: `osmnx`'s own rate-limiter and retry logic have unbounded internal waits that a URL change does not touch, so the fix had to be a transport swap with its own client (`graph/pbf_source.py`, #275), not a new default for `settings.overpass_url` | A discretionary "prefer the mirror when convenient" policy (erodes the same way the pre-`HotlinkRefused` tile path did — §10's own argument for why this has to be a gate, not a preference); eliminating public Overpass use entirely (gives up the live-refresh workflow §11.4 names, and the capped affordance costs nothing once bulk load is gone — Q5); routing every request through Overpass with a private, non-default endpoint (does not touch A23a's `osmnx`-internal unbounded waits, and keeps volunteer infrastructure as the load-bearing dependency for a commercial product); deferring the mirror until Phase 4/hosted web needs it (Phase 0's #232 IP-level block already showed the load was intolerable on the desktop-only MVP, before any hosted surface existed) |
| **D64** | **A trip has one stored travel-mode set (`Trip.modes`), and a passage's mode is a pick from it. The pre-#319 pair — a stored `declaredModes` the entry prompt wrote plus a segment-derived `modes` getter, with a per-passage picker offering every wire mode — collapses to the stored set: the entry prompt sets it, the Author adds to it from the trip-mode control on the planning page, a passage that arrives in a mode outside it (clone, older payload) adds the mode silently, and it never shrinks on its own — removing the last passage in a mode leaves the mode; only the Author removes one. The per-passage picker offers only the set, with "add a mode to the trip" on the control. No mode is preselected for a new passage. Stale rule: a change of parent mode (cycle → foot) marks the route stale; a change of discipline within a parent (road → gravel) does not. "Mode" means travel mode only — Explore/Compose is a posture with no "mode" label, and the appearance setting is not a mode** *(new — issue #319, 2026-09-18; restored as D64 under #446 after merge `320c10c` dropped it in a D63 collision with #287; the UX review #271's two-identical-selectors finding, answered by the owner on the issue)* | Two sets that could disagree gave the New route panel two identically-drawn selectors and let an Author believe changing the second changed the trip. A single set with the passage pick as a filtered choice makes ownership (multi-select) and choice (single-select from the parent) read as different kinds of thing, and removes the "which set is authoritative for layers" question (#316 keyed layer defaults off the stored set already). Never shrinking automatically is what keeps a mode the Author declared for a passage not yet planned from vanishing when an earlier passage in it is deleted. Persisted as the one `trips.modes` column (schema v6 folds the old two, declared order first); the payload schema is untouched — the set is local trip metadata, not canon. | Keeping both sets with the derived one authoritative for layers (the exact ambiguity #319 was filed on); an unfiltered passage picker that silently adds to the trip (Q13's disagreement case, ruled out by construction instead); a default passage mode from canonical order or `'cycling'` (a Plotlines 1.0 artifact — owner's call); shrinking the set when the last passage in a mode goes (a declaration is the Author's, not a projection of the itinerary); marking a discipline change stale like a mode change (owner's call — within-parent is not a change of parent) |
| **D65** | **Production basemap coverage is TTL-refreshed on-demand extraction from Protomaps' hosted daily build, through the mirror — never a purchased planet archive on zero-egress object storage.** `deploy/mirror/protomaps_extract.py` walks a small named-region list (today: the WNC corridor, primary, plus North Carolina full-state), skipping a region whose extract is within `--ttl-days` (default 30, `mirror_state.DEFAULT_BASEMAP_TTL_DAYS`) and otherwise re-probing/re-extracting it in place, safe to run daily from cron/systemd on the Pi. `SidecarUpstreams.tilesUpstream`'s built-in default now points at the mirror's primary region (`MIRROR_WNC_CORRIDOR_URL`) rather than staying unset *(new — issue #457, 2026-09-21; corrects `Plotlines_OSM_Acquisition_Review.md` §6.0/§6.3's basemap-tile reading of Q6)* | §6.0's Q6 = D + C ("clip server-side, keep whatever bulk remains on zero-egress object storage") was written for both halves of acquisition, but the basemap half of "D" — a purchased whole-planet Protomaps build on object storage — stayed unprovisioned by design (§6.0/Q6, §6.2: "do not put a planet archive on the Pi") with no mechanism to ever refresh even the one region the mirror does carry: the corridor extract's `MIRROR_STATE.json` entry had not moved since #394's one-time pull (`age_days: 628.5`, checked 2026-09-21). #456 removed the cost objection that made "just extract on demand" look expensive — the full WNC corridor bbox coalesces to 16 requests / 2.9 s against the mirror-clip path, and a live per-bbox `pmtiles extract` against a deeper archive costs more directory-resolution requests, not a different order of magnitude. Protomaps documents exactly this shape (`pmtiles extract` against a remote hosted URL, no full download) as a first-class use case, and `protomaps_extract.py` already used it this way for the one-time WNC pull; this decision makes it repeatable and multi-region rather than acquiring a static planet copy. **Q6's OSM-extract half is unaffected and unchanged** — `/clip` still clips Geofabrik state files server-side per trip bbox (D63), pinned bulk state files still live on the Pi's NVMe, not object storage; only the basemap-tile reading of "D" is corrected | Acquiring and hosting a full Protomaps planet build on zero-egress object storage as §6.0/Q6 originally named (still unprovisioned — no bucket, no credentials in this repo, and #456's measurement removed the performance argument that made per-bbox extraction look necessary); leaving the corridor extract a one-time, never-refreshed pull (exactly the "we become the availability" failure §11.3 warns about — a silently stale basemap is indistinguishable from a working one until an Author notices outdated tiles); reaching Protomaps directly from the client for a live per-trip extract (FR92 forbids it — the client only ever talks to Plotlines' own `/tiles`; this issue's refresh runs server-side, on the mirror, the same place `/clip` already reaches Geofabrik from) |
| **D66** | **A request thread on the service's shared pool makes no outbound call and takes no lock whose holder might be making one. Every outbound call runs on a named, dedicated pool behind a caller-side deadline (`future.result(timeout=)`), returns the honest degraded answer for that one capability on expiry, and treats a transport failure as transient — reported for this request, retried under cooldown-and-cap, never latched into a restart-only state. Long CPU work on a handler has the same rule. The client puts a timeout on every sidecar call, and both halves are owed because neither is sufficient alone** *(new — §8.6; issues #466, #488, and the post-#488 review #490–#497, 2026-09-22)* | Twice now an unbounded blocking call has taken the sidecar down on the same platform and the same leg — the DNS lookup no library timeout covers (#466 as a test that passed by resolver timing, #488 as `/health`'s 2 s poll filling the pool with stuck `MIRROR_STATE.json` fetches until `/layers` and `/tiles` could not get a thread). The review after #488 found the pattern live in five more places, each under a docstring that already explained why that call was safe: `regions.py`'s "the mirror removes the contention" (#434 says the shipped client does not pass the mirror flag), `extract_fetch.py`'s "an unreachable host still fails fast" (true of a refused host, false of a stalled resolver), `mirror_clip.py`'s "single process" (bounds nothing — anyio still hands out 40 threads). Prose does not hold this line; §10 and addendum P6 make the same argument about the endpoint list. The failure is misdiagnosed by construction — a spinner, `pending` forever, "No basemap tiles here" — so it costs a live-Pi session to find each time. The reference shape shipped with #488 (`_mirror_capability` on `Readiness._mirror_state_pool`, an 8 s deadline over a 5 s socket timeout) and is small enough to apply per outbound concern | Switching HTTP libraries (`httpx`, `requests`, `urllib` and `http.client` all end at the same `socket.create_connection`, whose `getaddrinfo` has no timeout — no swap closes the gap); `async def` handlers (an async handler calling a blocking library blocks the event loop itself, strictly worse, and every outbound library here is synchronous); a per-endpoint pool (the property that matters is that the *shared* pool never blocks — one pool per outbound concern is enough); raising the shared pool's capacity (delays the same failure and hides it longer); keeping the lock-across-socket shape for `OSM_SETTINGS_LOCK` / `_NOMINATIM_LOCK` because each lock has a good reason (the reason is bookkeeping — settings mutation, a 1 rps interval — and holding it across the round trip serialises the *wait*, not the mutation); leaving the rule in §8.6 without #497's gate (the docstrings above are what "held by review" produced) |
| **D67** | **The mirror fetches what it doesn't have. On a miss, the mirror asks upstream for the area and keeps the result, and the next request is a hit. Built as two parts that never mix: a *store* (fixed, immutable paths served as plain files — Caddy on the Pi, a zero-egress bucket when hosted) and a *fill worker* (one small service that takes a fill request, fetches from upstream, writes into the store and reports progress). A fill reports one of `fetching` / `ready` / `no_upstream_coverage` / `failed:<reason>`. The sidecar treats `fetching` as a retryable not-yet, never a final answer, and the client shows it as a waiting state distinct from failure and from out-of-coverage. Per layer: **OSM** — a `/clip` miss inside a Geofabrik-published region queues a pull (and precut) of the covering region under `geofabrik_pull.py`'s etiquette; a bbox outside every Geofabrik region is `no_upstream_coverage` at once. **Basemap** — on the Pi, a miss extracts the covering area from Protomaps' daily build into the store; once a bucket exists, the store holds the whole planet build and the basemap needs no fill at all. **Elevation** — the QA elevation proxy's existing fetch-and-keep behaviour becomes the contract's elevation fill; the production source is #148's. Pre-warm scripts (`prewarm_basemap_priority_regions.py`, `prewarm_priority_regions.py`) stay, as seeding for the store, not as the only way coverage grows** *(new — epic #516, 2026-09-24; supersedes D65's "small named-region list" as the limit of basemap coverage and `Plotlines_OSM_Acquisition_Review.md` §6.0's "everything beyond that one endpoint stays dumb"; D63's Geofabrik-not-Overpass rule, FR92's hotlink refusal and D66's request-thread rule are unchanged)* | #453's live retest (2026-09-24) went grey over Greensboro. The code was right: `corridor.pmtiles` holds nothing there past z7, and `/clip` answers `no_mirror_coverage` for anything outside the precut WNC corridor. Every document had called the mirror "on-demand", but that meant the sidecar's caches filling from the mirror (FR94, §8.1), never the mirror filling from upstream. Each layer's mirror half was decided alone against a fixed, hand-picked area list (§6.2 "regional extracts only", §6.6 "regions named explicitly", D65), so the only way coverage grew was someone running a script. Elevation was the one exception: the QA proxy already fetches on a miss and keeps the result forever, which is why the other two looked broken rather than undecided. Splitting store from worker is what keeps §6.0's rule that moving to hosting is "a hostname change": the store stays a bucket layout with no server-side logic, and the worker is deployed next to it. Holding the whole basemap planet once hosted is cheaper than any fill logic (~120 GB at zero egress); D65 turned it down for having no bucket, not for cost. A fill is minutes, not seconds (a full-state Geofabrik pull plus precut; full-state clips measured 627–640 s on the Pi, SPIKE-I), so it cannot sit inside a request: D66 requires the `fetching` answer and a poll, not a longer deadline. Fills are expensive and hit volunteer or quota-bound upstreams, so they are single-flight per area, gated by the mirror's client key and rate limit (#263), never startable by the anonymous web reader (SPIKE-F, D59), and bounded by a store size cap with eviction of the least recently read | Keeping pre-stocked named lists and widening them by hand (the #453 retest is what that looks like to an Author: grey, with no signal that anything could be done); fetching inside the request with a long deadline (D66 — a multi-minute outbound call on a request thread is exactly what #488 took down); putting fill logic in the file server itself (breaks §6.0's hostname-change property; a bucket cannot run it); a planet archive on the Pi (§6.2's storage rule still holds on the Pi — fill-on-miss grows toward the need, not the planet); the client or sidecar fetching from Protomaps/Geofabrik directly on a mirror miss (FR92 and D63 — upstreams are reached from the mirror only); treating `fetching` as `failed` with a retry button (makes the Author do the polling and hides the difference between "wait" and "won't") |
| **D68** | **Elevation voids split into gaps and absences (FR88/FR89 amended, #473).** A gap inside a raster that opened (`nodata`, NaN, `inf`, out-of-bounds) is linearly interpolated from the nearest finite samples — along-track for a route, graph-adjacency for enrichment — with `0.0` only when nothing finite exists to interpolate from. A raster that never resolved or will not open is *absent*: `ElevationResolver.sampler_for` returns `None`, a degraded sampler reads NaN / profiles `{}`, `enrich_elevation` annotates nothing and strips a stale annotation | The uniform `0.0` fill conflated two different facts. On real coverage, a nodata pixel turned into a drop to sea level and back, so a route's ascent/descent counted a plunge the terrain does not have. With no DEM at all it produced a flat profile indistinguishable from measured flat ground — #466 had already made the QA-proxy path report absent for this reason, and this makes the shared interface agree with it. Every reader already treats a missing `elevation` attribute as "no elevation" (`scoring.metrics`, `graph.loader._elevations`), so absent needed no new consumer contract. Still never raises, still no network in a solve (FR88) | Keeping `0.0` for gaps (the defect); nearest-neighbour hold for interior gaps (a step where the terrain is a slope, still distorts gain); interpolating the graph's node array in `graph.nodes` order (node ids carry no spatial order — the neighbours are the graph's edges, not the array's); a sentinel elevation value for absence (every consumer would have to learn it, and one that did not would compute gain against it); raising `ElevationUnavailable` out of `sampler_for` (breaks FR88's never-raise and every planning caller) |
| **D69** | **Phase-1 elevation is wired into region builds (#148): local cache, then OpenTopography direct when a key is in the environment; per-region elevation readiness; enrichment on a copy swapped in whole.** Source is chosen once at sidecar startup (`resolve_elevation_wiring`): the QA proxy flag wins outright, else a key wires `OpenTopographyClient` behind the cache, else cache-only. A missing key is not an error; an unrecognised tier or a posture the tier cannot serve is refused and runs cache-only with that refusal as the stated reason — never defaulted to the free tier. `capabilities.elevation` keeps its process-wide `ready`/`reason` and gains `regions`. An unreadable raster fetched this build is deleted from the cache | ARCH §12.1 already fixed Phase 1 as "each device → provider directly", and `keys.py` had enforced FR87 since 2026-08-30 with nothing calling it — every runtime region had no elevation, so FR2's peaks weight (which reads `grade_abs`) and every climb figure were inert off the committed fixture. Per-region readiness because one process-wide flag would read `ready` over a region whose bbox the allowance or the network left without data, and show a climb of 0 m. The copy-swap because routing is open on the graph before elevation runs (B1), and `scoring.metrics` sums climb over whichever nodes carry `elevation`. The delete because OpenTopography can answer 200 with an error body the fetch saves as `.tif`, which would otherwise be a permanent local "hit" | Shipping a builder's key in the app (one free-tier allowance shared by every install, and a secret in a distributed binary); falling back from the QA proxy to a direct call (defeats centralising calls, #304); a fixed elevation ETA (the `GRAPH_ESTIMATED_S` mistake, #397); enriching in place under a lock (every solve would take it); refusing to start the sidecar on a bad key configuration (elevation is never the reason planning stops, FR88) |
...
| Q8 | **Plugin distribution** — pub.dev + PyPI, or a bundled registry? **v2.0 splits this**: the *output* side stays Leg 7, but **data-layer distribution is now a Leg 2.5 question** and needs an answer sooner. **Python half answered by SPIKE-17 (2026-09-01): an ordinary installable package behind an `importlib.metadata` entry point, one group per direction — `curation/plugins.py`'s existing mechanism, which nothing in the spike argued against. The Dart/output half is still open** | ~~Leg 2.5 (data layers)~~ **closed for Python** / Leg 7 (outputs) |
...
| ~~**Q14** ★~~ | ~~**Driving-mode routing** — does the existing OSM graph and solver handle trailhead approaches acceptably, or does it need its own weights and constraints?~~ **Resolved 2026-08-31 by SPIKE-E (#171).** **The solver does; the graph configuration does not; and it needs *fewer* weights, not different ones.** Four approaches — two inside the shared fixture bboxes (Gregory Canyon Trailhead, Sidie Hollow landing), two genuinely remote (Big Sandy Trailhead WY, the Boundary Creek put-in ID), every destination a cited OSM element. Cold solves **2.8–14.5 ms** over 722–12,702 edges. But `network_type="drive"` is an **OSMnx download filter**, not a routing choice, and it excludes `highway=track` **and** `highway=service`: the shipped mode **routes 265 m short of Gregory Canyon Trailhead and reports success**, and retains **42.6%** of the road corridor within 15 km driving distance of Big Sandy (verified against a live `drive` pull at 99.61% way agreement). Driving's `WeightProfile` is inert at its own `directness=0.95` — `surface_paved` −1 → +1 changes no route in three of four approaches — and the flat `base_speed_kmh=60.0` is **44.6% short** on the canonical dirt approach and 19.7% long on the paved one. **Closes with a four-item bill, none of it a scorer** (driving download filter incl. `track`/`service`; way-tag list extended plus a decision on node-tagged `barrier`; `destination` out of driving's `excluded_values`; a tag-reading time model), so FR130's data-not-code extension path survives. **Two shipped defects surfaced:** `cues.route_polyline`'s edge spans do not tile the route (13.9–26.4% unattributed — **#205**), and `graph/regions.py` downloads neither `4wd_only` nor `motor_vehicle` and cannot carry `barrier`, which leaves `_BARRIER_DEFAULTS` unreachable for every mode (**#206**). See `spikes/SPIKE-E/results/RESULTS.md` and §7.4. | Done |
...
| Driving as a routed mode | FR29, FR10 | §7.4 (measured); **Q14 closed / SPIKE-E** — needs a `track`+`service` download filter, not the `drive` network type |
...
| Drive-leg access advisory | FR29a | §7.4 (measured); **Q14 closed / SPIKE-E** — honesty floor at the `read` band, merged sections, `4wd_only` opportunistic |
