...
| Q10 | **Region/bbox selection strategy** — fixed named regions, per-trip bounding box, or both — and the first-run experience for a Character with nothing downloaded. | **Sizing resolved by SPIKE-14 (2026-08-15): ~3.5 MB per 1,000 km² at z0–15**, over a ~1 MB floor; **1.0 MB** for a CI bbox, **22 MB** for an 80 km square (11.5 MB capped at z14), **118 MB** for a 235 × 134 km multi-day corridor. At that scale a default region is not a storage trade-off — pick the region that routes well. The measurements argue for **per-trip bounding box** (a corridor is an order of magnitude larger than a local region, and only the trip knows which corridor), but that is a product call, not a measurement, and **first-run with nothing downloaded remains unanswered.** |
