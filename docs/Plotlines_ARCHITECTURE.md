...
**Status:** Draft · **Version:** 1.0 · **Companion docs:** `Plotlines_PRD.md` (103 FRs / 102 stories — the source of truth for *what* and *why*), `Plotlines_Research_Spikes.md` (feasibility unknowns to prove before building). This document covers *how*.
...
**Where the list comes from (SPIKE-21, 2026-08-16).** This engine assumes a precomputed cue list on-device, and `plotlines-core/trips/cues.py` is what produces it (**D31**) — turns collapsed to real decision points, surface shifts, and the Author's nodes, hazards, portages and alternates interleaved by distance along the route. Two properties of that output this tier depends on: every cue carries `distance_along_m` against the *drawn* polyline (not the junction path, which measures short), and cues on a retraced spur carry `retrace` — SPIKE-01's lollipop means a rider can meet the same road twice, and an unmarked second pass is how they take a turn they were meant to ride past.
...
**What "plain data" means, concretely (SPIKE-20, 2026-08-16).** `Segment`, `Day` and `Trip` are the dataclasses in `plotlines_core/trips/payload.py`, and their JSON form is specified by **[`docs/schemas/trip_payload.schema.json`](schemas/trip_payload.schema.json)** — one document that is simultaneously this function family's return type, drift's stored blob (§10.3), the hosted `trip.payload` (§10.1), and the Flutter domain layer's source (§9.1). `compose_day` and `split_trip` live in `plotlines_core/trips/compose.py` and are **the only supported way to build a payload**, because the derived fields — B2's endpoint gap, C3's limit breaches, D1's roll-ups — are written there and nowhere else. Where the schema and any implementation disagree, the schema wins (**D28**).
...
**Four of those seven domain classes are the trip payload; three deliberately are not** (SPIKE-20). `Trip`, `Day`, `Segment` and `WeightProfile` deserialize from `trip.payload` against the schema named in §6.1. `RiderProfile` is account-scoped (§10.1's `rider_profile`), and `FieldNote` and `Amendment` are group-relay **layers over** the canon (P8) with their own tables and write paths — a field note can never live inside the trip blob. One layer in the diagram, two lifetimes underneath it.
...
- **`trip.payload`'s structure is specified, not described** (SPIKE-20, 2026-08-16): [`docs/schemas/trip_payload.schema.json`](schemas/trip_payload.schema.json), written in its canonical form — sorted keys, compact separators, no non-finite floats, `null` never written (absent means unset), SI units, `[lon, lat]` at 7 decimals. The sentence this bullet used to be ("segments, days, transitions, weight profiles, curated nodes, narration metadata, hazards") is still true and is now a table of contents rather than a contract. **JSONB does not preserve key order**, which is why the schema forbids key order from carrying meaning anywhere, and why any digest is computed over the canonical form rather than over stored bytes.
...
`payload` is TEXT holding the same canonical JSON as §10.1 (SQLite has no JSON type), against the same schema. Two rules SPIKE-20 measured rather than assumed:

- **List surfaces project columns; they never `SELECT *`.** drift's `select(trips)` returns the payload with every row, so drawing G2a's trip list pulled 20 week-scale blobs into memory: **137 ms**, against **1.0 ms** for an `id`/`name`/`updated_at` projection. G2a also wants each row's *modes*, which no column carries — add a denormalized `modes` column rather than decoding a megabyte per entry.
- **`dirty` is about sync, not staleness.** A segment whose authored inputs changed since it was solved carries `solve.stale` **inside** the payload; `dirty` and `updated_at` cannot express it, because they also move for a reworded note.
...
| **Basemap tiles** — **Protomaps Basemap** (OSM-derived, built with Planetiler; PMTiles archives extracted per bbox from the daily planet build and mirrored to our own storage — D23, FR95). Service contract — own-service-only, bbox-scoped on-demand — holds independent of source: FR92–94, D21 | Map rendering | Long (a basemap changes on the order of days; the daily build is the refresh unit) | **Required — ODbL.** `© OpenStreetMap` → `https://www.openstreetmap.org/copyright`. Distinct from the elevation layer's CC BY (FR86): the About surface owes both, under different licences |
...
| **USGS 3DHP** (waterway network — `3dhp.nationalmap.gov`, layer **50** `Flowline`; D27) | Paddling graph | Long (hydrography is static) | US public domain — credit requested |
...
**The waterway row changed source on 2026-08-16 (D27).** SPIKE-04 measured NHDPlus HR;
USGS retired the NHD on 1 October 2023 and stopped maintaining it. SPIKE-19 verified the
successor carries everything that verdict rested on and switched the row. Two operational
notes that cost an afternoon each if they are rediscovered rather than read: **the
flowlines are layer 50** (layer 1 returns HTTP 500 with the body `json`, which reads like a
transport failure), and **`featuretypelabel` must never be used to filter for real water** —
every major paddling river in the tested regions is typed `Waterbody Connector`, and the
share rises with stream order to 100%. Filter on `streamorder`.
...
`WaterwayDataProvider` is the seam SPIKE-04's answer plugs into, and the answer is now known: **implement it against USGS 3DHP (network) and the USGS Water Data APIs + NLDI (gauge, reach linkage) — not OSM** (§6.4, D27). Two consequences for its shape:

- **`WaterwayGraph` edges need *two* join keys — `mainstemid` and a reach code — not one.** They are the identifiers USGS gauges are indexed by, so carrying them turns "which gauge governs this segment" into a lookup rather than a spatial nearest-neighbour guess, which is wrong precisely at confluences and below dams. **This rule said `reachcode` alone until 2026-08-16, and SPIKE-19 found that unbuildable on the successor dataset**: 3DHP flowlines carry no reach code at all (it moved to hydrolocation layer 40), and measured over all 112 real-time gauges in three regions, `mainstemid` binds **77.8%** of NLDI-resolved sites and a reach code **80.6%** — but they fail on *different* sites, and together reach **94.4%**. Carry both, try them in that order. Two traps go with it: **`mainstemid` is a geoconnex.us URI in two disjoint registries** (`/usgs/mainstems/` and `/ref/mainstems/`; 0 of 933 numeric ids in the Western NC test appear under both), so match the full URI and never normalise the prefix away; and NLDI answers in the `ref` namespace while most flowlines are `usgs`. Dropping either key costs nothing until FR14, then costs a re-fetch of the whole network.
- **Topology is a downstream pointer, not node identifiers.** 3DHP has no `fromnode`/`tonode`; connectivity comes from `hydrosequence` / `dnhydrosequence`. It is still *declared* topology — the property that made this dataset beat OSM — but the graph must be built by **inverting `dnhydrosequence`**, which is one-to-many at confluences. `uphydrosequence` names only the *main* upstream path and would silently drop every tributary.
...
| A2 | **Paddling data may not exist** at the quality multimodal-MVP assumes (§6.4) | **HIGH → partly realised** | **SPIKE-04 ran (2026-08-14).** Network and gauge data are solid (USGS); **class ratings do not exist** in open data and the authoritative source prohibits reuse. The architecture is unharmed — the isolation behind `WaterwayDataProvider` (§13.2) is exactly what absorbed it — but one PRD acceptance criterion (B4/B5 class-band enforcement) cannot be met from open data. Residual risk moves to the **PRD scope call**, not the design. **SPIKE-19 re-tested this on the successor dataset (2026-08-16) and it holds**: 3DHP carries everything SPIKE-04 measured, a real directed route and identifier-bound live gauges come out of it, and the provider isolation absorbed a change of national data product without touching the core — which is the second time §13.2 has earned its place. The residual that remains is a **timing** one, not a quality one: the tested regions are still the converted NHD snapshot (`workunitid = NHD`), so when elevation-derived hydrography actually lands, geometry, Z and possibly topology all change and this risk should be re-opened rather than assumed closed. |
...
| A17 | ~~**The traffic-stress model (FR3, A2) infers stress from OSM highway class alone**, which overstates rural traffic~~ — SPIKE-03 measured a **35% traffic-exposure floor** on empty Viroqua county roads tagged `tertiary`/`secondary`, unreachable by any weight setting | **Low (was Medium)** | **Resolved 2026-08-16 (D33): rural/low-signal roads are the model's zero-stress baseline, not a highway-class floor.** Highway class alone no longer sets a floor; a road with no contrary signal starts at minimum stress, and the retained-but-previously-unused `maxspeed`/`lanes` attributes are what raise it above baseline. |
| A18 | **`WeightProfile` names three different structures across the PRD, this document, and the code** — the Author-facing `climbing`/`traffic`/per-class `surface`/`poi` on 0.0–5.0 (FR2–FR5); §6.3's documented `climbing`/`traffic`/`surface_pref`/`poi_bonus`/`detour_budget`/`terrain_technicality`; and `scoring/profile.py`'s implemented `quiet`/`surface`/`scenic`/`directness`/`peaks` on 0.0–1.0. Found by SPIKE-20 having to pick one for the payload | Medium | **Half-resolved: D28 fixes what the payload stores** (the Author-facing form), so no saved trip depends on the ambiguity. What remains is the conversion nobody has written and §6.3's field list matching neither of the other two. Cheapest fix, and it belongs with the first slider: write the mapping as one function in `scoring/`, and correct §6.3 to describe the implemented profile plus the Author-facing one it is derived from. Left as a risk rather than closed because until that function exists, every consumer is free to invent its own mapping |
| A19 | **Cue sheets promise surface shifts that OSM tagging often cannot supply.** SPIKE-21 derived **six** surface cues across 132 km of fixture routes: Boulder (81.7% of edges tagged) produced 4, **Davis (34.4%) produced none on any route**, and **Viroqua (24.5%) — genuinely gravel country — produced 2**. F1/FR46 lists surface shifts as cue-sheet content without qualification | Medium | **The rule is right and the data is thin, so the requirement is what moves.** Unknown tagging must never be treated as a surface value, and surface must never be inferred from highway class (the A17 overreach). Mitigations, in order of cost: state the limit in F1's AC; show tag coverage for the route's region beside the sheet so an Author knows what the silence means; longer term, let an Author annotate a surface change by hand, which is the same shape as FR15's Author-drawn portage. **FR4's `singletrack` class has no `surface` source at all** and needs a `highway`+`tracktype`/`mtb:scale` model before it can be cued |
...
| D21 | **Client-owned tile-service contract**: the client never contacts a third-party tile host directly, regardless of upstream tile source | Keeps licensing/attribution and rate-limit exposure entirely server-side and swappable; the dev-time OSM proxy is temporary implementation, this contract is not (PRD FR92). **SPIKE-14 independently vindicated this**: the `pmtiles` Dart package cannot be resolved alongside the only `vector_map_tiles` line that supports current `flutter_map` (`latlong2` and `protobuf` conflicts), so a client that read archives directly is unbuildable today — while the sidecar reads PMTiles in Python with no conflict | Client linking directly to a public tile host — breaks offline-first and puts licensing terms on every device; client reading the PMTiles archive directly — **now ruled out on evidence**, not preference |
...
| D27 | **The paddling network comes from USGS 3DHP, and `WaterwayGraph` edges carry two join keys instead of one** (2026-08-16, SPIKE-19) | The source this architecture named — NHDPlus HR — was **retired by USGS on 1 October 2023** and is no longer maintained; 3DHP is the single replacement. SPIKE-19 verified the succession is safe: `flowdir`→`flowdirection`, `streamorde`→`streamorder`, `fromnode`/`tonode`→`hydrosequence`/`dnhydrosequence`, `reachcode` relocated to hydrolocation layer 40, **nothing lost** — and produced a real **148.2 km downstream route on the French Broad** from 3DHP geometry with **five live gauges bound to it by identifier**, reproducing SPIKE-04's network length within **0.1%**. The two-key change is forced rather than chosen: with no reach code on the flowline, `mainstemid` alone binds 77.8% of gauges and a reach code alone 80.6%, and they fail on different sites (§13.2). **Two things this decision deliberately does not claim:** the data has not actually been remapped — every flowline in all three test regions still reads `workunitid = NHD`, so this buys a *maintained* product rather than better data; and **paddling routes must be solved downstream-directed**, since an undirected solve overstated Southwest Wisconsin's best route by 2× on a path no boat can follow | Staying on the archived NHDPlus HR snapshot (works today, stops improving, and defers a migration whose cost only grows once elevation-derived hydrography lands); keeping `reachcode` as the sole key (measured unbuildable — it is not on the flowline); normalising the two `mainstemid` namespaces into one (they are disjoint registries — 0 of 933 ids overlap — so it manufactures false joins); taking stream gradient from 3DHP's Z ordinate (advertised via `hasZ: true` and **0.0 on all 40,938 vertices sampled** — D20 holds unchanged, revisit when `workunitid` stops reading `NHD`) |
| D28 | **One checked-in schema is the contract for `trip.payload`, written in a canonical form** — [`docs/schemas/trip_payload.schema.json`](schemas/trip_payload.schema.json) (2026-08-16, SPIKE-20) | Three documents each described a piece of the same object and none of them was the object (§6.1's signatures, §10.1's sentence, §9.1's class list), and all 31 desktop-MVP stories write through that seam. SPIKE-20 implemented the schema three times over a real four-day multimodal trip solved on the SPIKE-01/02/03 graphs: core → JSON → drift → Dart domain classes → JSON, with **zero field-level loss** on all three regions and on a 45,912-vertex week-scale payload. Canonical form (sorted keys, compact separators, `allow_nan=False`, `null` never written, SI units, `[lon, lat]` at 7 dp, `additionalProperties: false`) is part of the decision rather than a style note: with it, Python and Dart emit **byte-identical files for the same trip**, which is what makes a content digest — the cue sheet's `derived_from.geometry_digest`, and SPIKE-21's staleness check — verifiable across the boundary. Where any implementation disagrees with the schema, the schema wins | Prose contracts in three documents (the status quo, and the thing MVP §1.4.5 called blocking); code-first generation from the Python dataclasses (makes the core's implementation the spec, which the Dart side cannot read); per-boundary adapters (three shapes to keep in sync, and the drift between them is invisible until it ships) |
| D29 | **The payload stores the Author-facing weight profile (FR2–FR5's 0.0–5.0 form), never the solver's internal form, and never both** (2026-08-16, SPIKE-20) | `WeightProfile` currently means three different things — the PRD's Author-facing 0–5 scales, §6.3's documented field list, and `scoring/profile.py`'s implemented 0–1 `quiet`/`surface`/`scenic`/`directness`/`peaks`. A payload has to pick one, and only the Author-facing form is stable under a change of scoring implementation: it is what someone set and what a UI redraws. The solver's form is derived on the way in (`w = ui / 5.0`; `(ui − 2.5) / 2.5` for the bipolar climbing term) | Storing the solver's form (a re-tuned scorer would silently reinterpret every saved trip); storing both (two representations of one preference — they disagree the first time one is edited without the other, which is the defect D5's "one structure" rule exists to prevent) |
| D30 | **`solve.stale` — the payload records when its derived half no longer matches its authored half** (2026-08-16, SPIKE-20) | `trip.payload` holds authored inputs (via-nodes, weights, target distance) and derived outputs (geometry, metrics, elevation, roll-ups, cue sheets) in one object, and an Author edit invalidates the second half instantly. Nothing recorded that: drift's `dirty` is about sync and `updated_at` moves for a reworded note too. Without the flag a client either re-solves on every keystroke or shows the previous route's numbers as if they described the edited one. Found by performing the edit in SPIKE-20's round trip, not by inspection. The client sets it; only a solve clears it | Re-solving on every edit (a solve is 27–218 ms and a keystroke is not); dropping derived fields on edit (D1's dashboard would go blank mid-edit, and the previous numbers are still the best available); inferring staleness by hashing the inputs on every read (the same information, recomputed constantly, and still needs somewhere to be cached) |
| D31 | **Cue derivation runs in `plotlines-core`, not the client** — `trips/cues.py`, beside `compose_day` (2026-08-16, SPIKE-21) | A cue sheet is derived from graph attributes the client does not hold: junction degree (is this bend a decision?), `name` and `osmid` (is the rider still on the same way?), and `surface`. A client-side implementation would need the graph shipped to it, and sidecar and hosted deployments would then produce different sheets for the same route, which P1 exists to prevent. It is also cheap enough not to argue about — **1.1–9.2 ms per route** for 74–1,127 polyline vertices, run once per solve. The Field Runtime's §5.3 engine consumes the result; it does not produce it | Deriving in the client (splits the algorithm across two languages and two deployments, for a computation that costs single-digit milliseconds); deriving on demand in the Field Runtime (the field tier must work with the sidecar dead — a cue sheet has to be in the payload before the ride starts) |
| D32 | **Region/bbox selection is three distinct shapes, not one model** (2026-08-16) — shipped default is a **rectangular bounding box of Buncombe County, North Carolina**; **first-run** (an Author with nothing downloaded) is a **100 km radius circle centered on a location the user enters** (city + state, zip code, or country + city — PRD A10); **per-trip** download extent is exactly the **Author-set corridor buffer** (C14/FR35), with no separate selection step | Resolves Q10's three open cases independently, since one model does not fit all of them: the shipped default only needs to exist and route well (PRD §8, SPIKE-14 sizing), first-run needs to center on the user rather than on a fixed named region, and a trip's download need is already fully expressed by the buffer the Author draws | A single fixed named region for all three cases (wrong for first-run — an Author elsewhere gets an unfamiliar, possibly unroutable area); deriving first-run from IP geolocation instead of a user-entered location (imprecise, and skips the direct prompt PRD A10 already commits to); a per-trip bbox computed independently of the Author's buffer (redundant input for an extent already captured) |
| D33 | **Rural/low-signal roads are the traffic-stress model's zero baseline, not a highway-class floor** (2026-08-16) | SPIKE-03 found stress inferred from OSM highway class alone gave rural Viroqua a 35% floor on empty `tertiary`/`secondary` roads, unreachable by any weight setting — a road read as loaded from its tag alone, with zero measured traffic. The fix keeps class as a signal but removes it as the floor: a road with no contrary signal starts at the model's minimum stress value, and stress rises only as the already-retained `maxspeed`/`lanes` attributes (risk A17) indicate real capacity/speed. Restores A2's "quietest possible rural route" case without discarding highway class where it remains informative (genuinely busy roads) | Leaving the class-derived floor as measured (directly blocks A2 `[MVP]`'s stated case); dropping highway class entirely in favor of `maxspeed`/`lanes` alone (those tags are sparse in the same rural regions per A19's coverage finding, so class remains the only signal present on many ways) |
...
| ~~Q9~~ | ~~**Concrete tile-generation tooling** — `tilemaker` → MBTiles, or an alternative?~~ **Resolved by SPIKE-14 (2026-08-15): we do not generate tiles.** `pmtiles extract` pulls a bbox out of the published Protomaps planet build over HTTP range requests — an 80 km square in **5.9 s / 76 requests / 22 MB**, without downloading the 128 GB planet. Running Planetiler ourselves costs hours of compute and buys nothing until we need layers the Protomaps schema lacks (e.g. baking Plotlines' own surface/traffic attributes into the basemap). Mirror the daily build to our own storage — which the licence guidance requires anyway (D23) — and extract per bbox on demand, which *is* FR94. | Done — Planetiler stays on the shelf for custom layers |
| ~~Q10~~ | ~~**Region/bbox selection strategy** — fixed named regions, per-trip bounding box, or both — and the first-run experience for a Character with nothing downloaded.~~ **Resolved 2026-08-16 (D32): three distinct shapes, not one model.** Sizing was already resolved by SPIKE-14 (2026-08-15): ~3.5 MB per 1,000 km² at z0–15, over a ~1 MB floor; 1.0 MB for a CI bbox, 22 MB for an 80 km square (11.5 MB capped at z14), 118 MB for a 235 × 134 km multi-day corridor — at that scale a default region is not a storage trade-off. **Shipped default** is a rectangular bbox of Buncombe County, NC. **First-run** (nothing downloaded yet) is a 100 km radius circle centered on a location the user enters (PRD A10 — an Author-Desktop story, not a Character/field-execution one). **Per-trip** download extent is exactly the Author-set corridor buffer (C14/FR35) — no separate selection step. | Done |
