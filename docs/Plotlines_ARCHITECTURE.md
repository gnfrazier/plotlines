...
| A15 | **No basemap labels render** — no street or place names on any current renderer version, because the Protomaps themes drive labels through `format`/`case`/`is-supported-script` expressions `vector_tile_renderer` does not implement (SPIKE-14) | Medium | A usability problem for an Author placing Nodes, not a cosmetic one. Four options, none yet chosen: a Protomaps **v3** theme (older expression syntax), a reduced Plotlines-authored theme with simple label expressions, contributing expression support upstream, or drawing labels as Flutter widgets from the `places` layer already present in the tiles we receive. Decide before the map screens are built on. |
