...
☑ **2A.3 — Candidate extraction depended on a public endpoint that varies ×21 and can withdraw service — resolved 2026-09-17 (#287), recorded as ARCH D63.** *(Mechanism corrected 2026-08-28 — SPIKE-D; severity raised to HIGH at the time.)* The historical reading — "candidate extraction is a heavier Overpass query than graph building" — was already measured backwards by SPIKE-D (a candidate pull is ×0.43 the time and ×0.78 the bytes of a graph build over the same bbox); what actually bit was throttling variance (the identical 176 km² query took 8.8 s and 185.9 s an hour apart) and single-endpoint dependence. **The line this item stood on — "local extracts remain the only mitigation that removes the variance; still unmeasured" — is now measured**, by SPIKE-I (#265, 2026-09-13), and Phase 3 (#272, closed 2026-09-17) shipped it: both `ensure_graph` and `OsmLayerProvider.fetch` resolve from **one shared mirror-side clip** before either calls Overpass at all, so the primary path no longer depends on a public, volunteer-operated instance. The measurement itself was not the happy number SPIKE-D's reordering implied — the clip costs 432–553 s / 1.9–7.2 GB, O(region extract) not O(bbox), and the two-extract merge failed 0/4 times — but both causes were found and fixed the same day (**#375** precuts pinned extracts at pull time, **#376** merges clipped outputs rather than raw extracts); the fix was **re-measured on the live Pi 2026-09-18 (#402, PR #438): 617 s → ~102 s on the same Asheville cell, an 83% reduction that tracks the 6.4× drop in pinned-extract size, still ~1.7× over SPIKE-I's ≤60 s outer band — the residual is #439**, and the same pass fixed rectangle-based over-selection (each extract's `.poly` boundary) and a live header-box defect on precut output. Public Overpass is retained only as the fallback when no clip is cached and for Phase 5.2's capped live-refresh affordance (#285, not yet built) — see ARCH **A23** (restated) and **A23a** (revisited: the `osmnx` rate-limiter/retry defects are unpatched, exposure shrank rather than the defect closing) for the current risk reading. `docs/Plotlines_OSM_Acquisition_Review.md` and its licensing addendum, accepted 2026-09-03, are executed through Phase 3 — Phase 0 #254 (closed 2026-09-04), Phase 1 #264 (closed 2026-09-13), Phase 2 #268 (closed 2026-09-16), Phase 3 #272 (closed 2026-09-17); Phase 4 #279 and the remainder of Phase 5 (#283: #284–#286) are still open.
*Fail signal (retired):* extraction with no bbox-scoped cache, no backoff, or a single hardcoded public endpoint — the shipped state has none of these; the residual watch item is #439 (`/clip` measured at ~102 s on the live Pi, still over SPIKE-I's band), not a correctness gap.
...
☐ **4.34 The paddling day's layers.** Create a trip declaring paddling only, and another declaring cycling only. Verify the layer picker's initial state **differs** and **states which categories it derived from**. Then, on the cycling trip, add hiking to the trip's categories from the planning page and add a hiking passage in it: verify the category is added **without warning or confirmation**, the passage succeeds, and the passage picker **offers only the trip's categories**. Change the category set and verify overridden days keep their overrides. *(PRD FR144, N0, FR97; #319)*
...
- **Add the driving-surface tags** — `surface`, `smoothness`, `tracktype`, `4wd_only`, `motor_vehicle` — which FR29a needs and the mapping does not carry. **Measured 2026-08-31 (SPIKE-E): this is a code gap as well as a markdown one, and it reaches past FR29a — filed as [#206](https://github.com/gnfrazier/plotlines/issues/206).** `core/plotlines_core/graph/regions.py` extends osmnx's `useful_tags_way` with `surface`/`tracktype`/`smoothness` and stops — **`4wd_only`, `motor_vehicle`, `motorcar` and `ford` are never downloaded**, and `barrier` is tagged on the *node*, where `useful_tags_node` carries nothing. So on a graph the product builds today: two of FR29a's six signals are unreadable; `MODE_CONSTRAINTS["driving"]`'s `access_key="motor_vehicle"` silently degrades to the generic `access=*` rule (53 ways carry `motor_vehicle` in SPIKE-E's four pulls); `ford_passable=False` never fires (5 ways carry `ford`); and **`routing/access.py`'s `_BARRIER_DEFAULTS` is unreachable for every mode**, including driving's gate/bollard row — the case ARCH §7.4 names as the reason a driving leg is a route rather than a note. A tag not requested at download time is not recoverable without re-downloading (`spikes/shared/README.md`'s "osmnx tag trap", found on the cycling side for `surface`).
...
☑ **SPIKE-E · Driving-mode routing and access advisory.** ~~Confirm the OSMnx graph and existing solver handle driving to trailheads acceptably, and **measure `surface`/`smoothness`/`tracktype`/`4wd_only` coverage on approach roads in the target regions** — the advisory is only as honest as the tagging behind it.~~ **Done 2026-08-31 (#171, `spikes/SPIKE-E/`) — the solver passes, the graph configuration fails, and §7.4's "cheapest new mode… the OSM road graph is already built" is why nobody looked.** Four approaches: two inside the shared fixture bboxes (Gregory Canyon Trailhead; Sidie Hollow landing) and two genuinely remote (Big Sandy Trailhead, WY; the Boundary Creek put-in, ID), every destination a cited OSM element. **`network_type="drive"` is an OSMnx *download filter*, not a routing choice**, and it drops `highway=track` **and** `highway=service` before a way reaches the graph — so the shipped mode **routes to 265 m short of Gregory Canyon Trailhead and reports success** (the 3 km `OutsideGraphExtent` guard from #154 never trips), and keeps **32.5 of 76.3 km — 42.6%** of the corridor within 15 km driving distance of Big Sandy. Verified against a live `drive` pull at **99.61% way agreement**. **The engine is fine** (cold solves 2.8–14.5 ms) **and the weights are decoration**: at driving's `directness=0.95` every other weight is applied at one twentieth, and `surface_paved` −1 → +1 changes **no route in three of four approaches**. **Time is wrong where FR29 aimed**: flat 60 km/h reports 71.8 min against 129.6 surface-aware on Big Sandy (**−44.6%**) and +19.7% on the mostly-paved Middle Fork. **Advisory half:** `4wd_only` is **0 tagged ways in 12,069 / 2,419 km**; `smoothness` is `absent` at network scale (0.3–4.3%) and **100% of Big Sandy's last 3.4 km**; `surface` carries the feature. The prototype **flags without ever rerouting** (identical edge walk before/after, all four) and merges **65 raw runs → 1 section** across unsurveyed gaps. **The requirement-changing finding is the honesty floor**: at the 20% band a 35%-surveyed gravel approach reports "no signal exceeds 2WD" — the confidently-wrong state, live — and the seeded thinning model puts that outcome at **18.8% against a pre-declared 10% ceiling**, falling to 0.0% at the 70% band. **Q14 closed** with a four-item bill (driving download filter incl. `track`/`service`; tag list extended; `destination` out of driving's exclusions; a tag-reading time model), **none of it a second scorer**. **Two shipped defects found ([#205](https://github.com/gnfrazier/plotlines/issues/205), [#206](https://github.com/gnfrazier/plotlines/issues/206)):** `cues.route_polyline`'s edge spans do not tile the route (13.9–26.4% unattributed, so `surface_min_run_m` suppresses real runs), and `graph/regions.py` downloads neither `4wd_only` nor `motor_vehicle` and cannot carry node-tagged `barrier` — which makes 5.3's last bullet a code gap, not a markdown one, and leaves `_BARRIER_DEFAULTS` unreachable for every mode. No product code changed. *(FR29, FR29a, C13, C13a; ARCH Q14)*
...
☑ **SPIKE-H · `LayerProvider` contract validation at Leg 2.5.** *(Added to this list 2026-08-28. It ran as issue [#160](https://github.com/gnfrazier/plotlines/issues/160) without ever having an entry here — at the time the series was **A–F and H, with no SPIKE-G**; SPIKE-G was created the next day by [#161](https://github.com/gnfrazier/plotlines/issues/161) to own ARCH Q15, and has its own entry below.)* ~~Validate that ARCH §14.2's `LayerProvider` is buildable by a real external dataset before B4 commits the contract at Leg 2.5.~~ **Done 2026-08-28 (`spikes/SPIKE-H/`) — positive; the contract holds.** Built §14.2's full shape as a spike-local prototype (no core change) and ran it against the real `OsmLayerProvider`/`TAXONOMY` plus **three live ArcGIS MapServer REST sources** — no synthetic plugin data anywhere. **Built-in layers express as `LayerProvider`** (290 real candidates across all six), which is the test §14.2 sets for itself. **One recorded bend:** `fetch_candidates(bbox)` takes no `layers` argument, so read literally it wants six provider instances while `OsmLayerProvider` answers all six in one Overpass call — a shared-fetch wrapper reconciles it, and a real one-dataset plugin never meets it. **Per-layer load state works against a non-cooperative upstream:** a real `404 Layer not found` reported as `failed:…` with every other layer unaffected — the §8.3/N2 behaviour the shipped app currently inverts. **D47 proved on real heterogeneous data:** 290 OSM + 82 NPS candidates → 30 proposals with **zero changes to `colocate.py`**, 14 of them mixing NPS with non-NPS and 2 carrying `station` affinity from a real declared type. **What this hands N2:** the shipped `providers.py` diverges from §14.2 (`licence: str`, no `taxonomy`, no `load_state()`, `fetch(bbox, layers) -> list[RawFeature]`), and because that returns a bare list, per-layer failure has nowhere to live — the cause of one layer 422-ing the whole extraction. The prototype is the target shape. **Reconciled into ARCH §14.2, §4.3, D47 and Q11 on 2026-08-28.** *(FR100, B4, N2, N5; ARCH §14.2)*

☑ **SPIKE-G · Candidate and proposal rendering at bbox scale.** *(Added to this list 2026-09-18, retroactively. Created 2026-08-29 by [#161](https://github.com/gnfrazier/plotlines/issues/161) to own the starred-but-unassigned ARCH **Q15** — §2A.5 above already records the resolution — and run the same day.)* ~~Over a realistic trip bbox at SPIKE-A's post-calibration densities, measure frame time, memory and selection latency with candidates and area anchors on screen over SPIKE-14's basemap; compare clustering-for-display, zoom thresholds, and salience-gated rendering; re-state A16 with candidates displayed.~~ **Done 2026-08-29 (`spikes/SPIKE-G/`) — salience-gating wins, and it is the only one of Q15's three options that satisfies FR99/N4a intent rather than just the frame budget.** Priced at 715 / 72 / **1,208** candidates (`avl` / `lwr` / `sgv`), 79 / 20 / 100 of them polygon area anchors. **The naive one-widget-per-candidate layer breaks** at the trip-overview zoom (`sgv` p95 **33.5 ms** against a 16.7 ms budget on a GPU desktop; 50–75 ms on the software-raster floor). **Zoom-thresholding blanks the overview** — the one zoom the co-location analysis runs over (hard fail, FR99/N3). **Clustering fits but aggregates salience into a count glyph and makes a map tap ambiguous** (fails FR99's "salience visible" and N4a's promote-from-map) — right as the zoomed-out affordance and the over-ceiling backstop, wrong as the primary mode. **Salience-gated**: the top-K in-viewport candidates by salience as hit-testable widget markers, the rest as a salience-styled canvas-dot field; K *derived* from SPIKE-14's warm-frame headroom, **≈300 on a GPU desktop**, ~0 on the software-raster floor. Selection latency is a function of the live widget count, not density (`map→list` ~55 ms naive vs ~20 ms gated; `list→map` O(1)). Areas (FR108) are ~1–2 ms/frame on GPU and dominant on `llvmpipe`. **Display-density ceiling ~2,800 candidates per viewport** on GPU; the shipped ruleset's worst trip bbox is 1,208, the pre-calibration flood was 5,453 — so clustering is the backstop against a ruleset regression, not a routine mode. **A16 restated at ~1.15 GB** (basemap ~1 GB + ~3 MB candidate layer + ~150 MB tile/transient headroom): the candidate layer is not a material memory cost and A16's lever stays tile-cache bounds. **A model calibrated to SPIKE-14's measurements, not a fresh measurement** — the live two-platform Flutter run is specified in `spikes/SPIKE-G/HARNESS.md`; the shape of the answer is robust to a 2–3× error in the per-marker coefficient, the exact K and ceiling are not. No product code changed. **Re-validated 2026-09-17 by #276 on SPIKE-I's bit-identical graph: ceiling and A16 unchanged.** Q15 closed; 2A.5 ticked. *(FR99, FR102, FR105a, FR108, FR120; N3, N4a, O3; ARCH Q15, A16)*

☑ **SPIKE-17 · Community edge-data extensions and API normalisation.** *(Added to this list 2026-09-01, when it ran as issue [#176](https://github.com/gnfrazier/plotlines/issues/176) — a Leg 7 spike, listed here because its normalisation sub-question reaches back into **P3**, an MVP principle.)* ~~Implement an edge data-input provider against §14.2's interfaces without changing core code; measure fetch-and-annotate against a real graph build, the TTL the data's volatility justifies, and what a stale annotation surfaces to the Author; decide whether normalisation needs a server tier.~~ **Done 2026-09-01 (`spikes/SPIKE-17/`) — positive, and the P3 question answers "no server".** Two real, deliberately unlike `EdgeDataProvider`s — **511 WI's WZDx 4.2 work-zone feed (4,424 events, 10.2 MB)** and **NWS active alerts** — annotated **two real graph builds** through the shipped `graph.regions.ensure_graph` (rural Driftless 777 km²/42,450 edges; Milwaukee 270 km²/138,979) with **zero core changes**. **Timing is not a design constraint:** fetch + match + annotate **0.67 s / 2.25 s** against builds of **17–91 s** (×0.013–0.13), and build variance alone is ×5 — A23 again. **No P3 design event:** a per-source adapter is **~30 LOC** (WZDx 27, serving *two* publishers with no branch; NWS 33) while the shared map-matcher is 90 and is device-side regardless. The loudest proxy argument — NWS geometry-by-reference, **62 requests / 11.3 s to place 5 alerts** — instead showed `EdgeDataProvider` is the **wrong seam for area-scale sources**: once placed, those polygons annotated **100%** of the graph. **Two §14.2 revisions required before Leg 7 publishes the contract:** annotation Protocols need `licence` and `load_state()`. Real evidence, not hypothetical — of two conformant WZDx feeds, **511 NY declares no licence**, so D45 says it must not load and §14.2 has nowhere to refuse it; and without `load_state()` an unreachable source is indistinguishable from "no work zones here", which for an advisory reads as all-clear. **`update_frequency: 60` s describes the pipeline, not the data:** 0.0% churn over three 60-second polls, event ages **p50 17.8 days / p90 105 days** → recommended TTL **6 h**, and **no source honours conditional GET** (all 200 + full body against their own ETag; 511 WI serves 10.2 MB uncompressed). **A stale annotation needs two clocks** — information freshness vs the event's own end date — and **neither belongs on M13** (D53); the age stamp beside the advisory is FR66 generalised. **Publication shape beats spec version:** point-published events recover **12.9%** of the line match's edges and put **46.5%** of their claims on the wrong road. **Names are a report, never a filter:** DOT↔OSM agreement **49.2% rural, 16.5% urban**. **P6 asserted on the bytes** — 0 of 18 core-consumed edge keys mutated; annotations live under `advisory:` and nothing in core reads them, which is what keeps FR14/FR29a advisory rather than FR128 constraint. **The source the spike named had moved and become keyed** in the five days between writing and running, answering `200` with a message body — so a naive client silently gets zero events. **ARCH Q8 closable for the Python half** by reusing `curation/plugins.py`'s entry-point mechanism. No product code changed. *(FR84, FR3/D33, FR14, FR29a, FR66; ARCH §14.2, Q8, P3, P6, P7, D45, D53)*

☑ **SPIKE-I · Local extract and graph parity.** *(Added to this list 2026-09-13, when it ran as issue [#265](https://github.com/gnfrazier/plotlines/issues/265) — a Phase 2 spike of the OSM acquisition plan, epic [#268](https://github.com/gnfrazier/plotlines/issues/268).)* ~~Confirm the graph that comes out of a mirror-clipped `.osm.pbf` is the same graph Overpass produces, before Phase 3 swaps the transport under `ensure_graph`.~~ **Done 2026-09-13 (`spikes/SPIKE-I/`) — RESCOPE, and the failure is not where §11.1 put it.** **Bands pre-registered and committed before the first measurement** (addendum G5/2b; the git history shows the ordering), with snapshot drift controlled by an attic-dated Overpass query at the extract's own `osmosis_replication_timestamp`, verified live so a silently-ignored `[date:]` could not bias every band toward a pass. **The spike split §11.1's risk into two swaps the review describes in one paragraph, and they answer oppositely.** *Transport swap* (clipped pbf → Overpass-shaped elements → osmnx's **own** `_create_graph` and the identical post-download pipeline) is **exact**: identical node and edge sets, **100%** edge-key stability, identical largest SCC, **0.0 m** max per-edge length delta, **zero** `PLOTLINES_WAY_TAGS` losses, **zero** way-filter disagreements — on `bike` **and** on `drive`, where it reproduces SPIKE-E's `track`/`service` drop exactly rather than accidentally fixing it. *Reimplementation* (pyrosm) is **5.6×** the node count on bike and **16.4×** on drive. So §8's "replaces the transport, not the interfaces" holds **literally**, the Phase 3 recommendation is specific — *swap the transport, keep osmnx's builder* — and the SPIKE-A/SPIKE-G/SPIKE-21 re-validation §11.1 told us to budget for is **not owed for the graph**. **What fails is §6.7's clip**, a Phase 1 component the review treated as settled, measured server-side on the real Pi 5 per addendum 2c: cost is **O(region extract), not O(bbox)** — 432 s / 2,853 MB for a 64 km² bbox out of a 428 MB extract, 553 s / 4,403 MB for a 27 km² bbox out of a 1.33 GB one, a bbox 6× larger in area costing 6.8% more time — against a pre-registered ≤20 s / ≤60 s band ([#375](https://github.com/gnfrazier/plotlines/issues/375)); and the **two-extract path does not complete at all** (0 of 4; `MergeInputReader` buffers both extracts, +3.9 GB in 5 s, dead in 8.8 s, **502** to the caller), which because `select_covering_extracts` compares *rectangular* header boxes fires for **every western-North-Carolina bbox** — §11.7's border case is the default for the flagship region, not an edge case ([#376](https://github.com/gnfrazier/plotlines/issues/376)). **Strategies implemented, not flagged** (addendum L1, since osmium-tool is GPL-3.0): `smart` costs **+54%** wall time for exactly one extra node; `simple` cannot reach parity because Overpass's `(way…;>;)` has been returning *complete ways* all along; the shipped `complete_ways` is correct and is the **floor**. ~77% of the clip's memory is the `BackReferenceWriter` pass, not the node-location table, so a file-backed index recovers only 6.6%. **Verified positives:** no GPL `osmium` binary anywhere in the running image (L1/2a's acceptance criterion, checked on the container) and every `/clip` 200 carrying its ODbL notice and `Link: …; rel="license"` (#364), both checked live rather than by reading code. **Two harness predictions were refuted and are recorded rather than tidied away** — the 500 m query buffer is not load-bearing (so the raw-bbox clip `/clip` already returns reaches parity unaided), and clip memory is sublinear in extract size, not the ~10 GB projected. `bands.py` encoded neither prediction, which is why being wrong cost a verdict clause rather than the verdict. No product code changed. **Feeds #287 (A23/A23a and 2A.3), #273/#278 (Phase 3), and #281 (Phase 4.2 concurrency).** *(FR120, FR121; ARCH A23/A23a; OSM Acq. Review §6.7, §7.1, §8, §11.1, §11.5, §11.7, §12-Q1/Q6)*

☑ **SPIKE-J · Packaging the native OSM dependency.** *(Added to this list 2026-09-18, retroactively. Filed 2026-09-03 as [#266](https://github.com/gnfrazier/plotlines/issues/266) — the second Phase 2 spike of the OSM acquisition plan, epic [#268](https://github.com/gnfrazier/plotlines/issues/268), sibling of SPIKE-I above.)* ~~Freeze the `.osm.pbf` reader inside the sidecar on all four targets — Linux, Windows, macOS x86, macOS arm — and report build / launch / read / graph-build per target with the size delta against A5's 150–300 MB budget; choose pyosmium vs. pyrosm on freeze evidence; add addendum 2a's no-GPL-binary check as a mechanical gate.~~ **Done 2026-09-13/15 (`spikes/SPIKE-J/`) — PARITY on all four targets; pyosmium ships.** Smaller than drafted twice over: Q1-C removed the client-side clip, so only the *reader* had to freeze, and SPIKE-I had already closed the dependency choice (pyosmium bit-identical to golden osmnx, pyrosm 5.6×–16.4× node-inflated), so only pyosmium was frozen. Only Linux exists locally; the other three legs ran on GitHub-hosted runners via a spike-only workflow (`.github/workflows/spike-266-freeze-matrix.yml`), each building the real `packaging/build_sidecar.sh` sidecar and a probe fork (+osmium) on the same runner the same day so the delta is attributable. **Freeze matrix pass ×4**: every target built, launched `"frozen": true`, read a `.osm.pbf` through pyosmium's C reader and handed nodes/edges to osmnx's own pipeline. **Size delta +3.4–4.5 MB uncompressed per platform** against a 20 MB band (Linux compressed +0.99 MB); the Windows and arm64 legs reproduced their deltas byte-for-byte two days apart. **A5 restated with numbers**: sidecar totals with osmium are 214–279 MB, all inside A5's range, osmium **1.5–1.9 %** of it — A5 stays Medium, range confirmed not widened. `check_no_gpl.py` passed on every target after three false positives were found and fixed first (pandas's licence-history prose, PyInstaller's own GPL-2.0 build toolchain that never ships, pyosmium's `osmium/` *directory* matching a check written for the GPL-3 CLI *binary*). Dependency tree recorded as the input to L5/2d's notice bundle (#267). **Infrastructure lesson**: `macos-13` is retired (the label parses, never schedules; the job sat queued 24 h and auto-cancelled twice) — `macos-15-intel` is the remaining Intel image, and a re-run should move `macos-14` to `macos-15`. **Consequence taken by Phase 3 (#275)**: pyosmium is a base dependency of `core` and `service`, `build_sidecar.sh` collects it and runs the promoted `check_no_gpl.py` after every freeze. *(OSM Acq. Review §7.2, §8, §11.2; addendum 2a/L1/L5; ARCH A5)*

☑ **SPIKE-K · Swappable basemap style JSONs against the Protomaps corridor extract.** *(Filed 2026-09-19 as [#461](https://github.com/gnfrazier/plotlines/issues/461), ahead of Epic [#458](https://github.com/gnfrazier/plotlines/issues/458)'s #456/#457 — implementation-informing, gates nothing.)* ~~Load #394's real corridor extract from the mirror into a throwaway MapLibre GL JS harness, check in five candidate style JSONs switchable live with no rebuild, and report which render cleanly against the Protomaps schema, which need schema changes and exactly what, side-by-side at the same camera.~~ **Done 2026-09-20 (`spikes/SPIKE-K/`) — positive; OSM Liberty/Bright swapped out on the schema check, as the issue anticipated.** All five Protomaps flavours (Light, Dark, Grayscale, White, Black — upstream's own generator, `@protomaps/basemaps` 5.7.2, unmodified) render against the v4.15.2 extract with **zero schema changes**. Liberty/Bright are a port, not a patch: 11 of 14 source-layers absent from a Protomaps archive (97/109, 103/118 layers dead), the three shared by name filtered on the wrong taxonomy, and the one layer that draws — an unfiltered `water` fill — triangulates Protomaps' waterway lines into slashes. Swap measured at 340–420 ms per full `setStyle`, no reload, 0.0 camera drift over 21 in-page switches; the shipped client already swaps Light↔Dark by name (`MapTileAssets.theme`). **#457's default: Protomaps Light paired with Dark** — the two flavours with a sprite and with `pois`/`landcover` layers; the other three are POI-less and landcover-less upstream, Black loses park fill entirely. SPIKE-14's label transform applies to every flavour mechanically. Beyond the question: the mirror sends no CORS headers (a wall only for a browser reader — #463), and no flavour carries terrain (relief is the elevation seam's, not a style's). *(PRD FR95; ARCH D22–D24)*
...
- **A list is a seed set; state the rule beside it.** Per §0. Applies to notability rules, role affinity, Set detection, routability constraints, the **discipline registry** (#315 — the rule: a discipline is a travel-mode *category* + a `WeightProfile` + a `grades_difficulty` capability flag; `multimodal/disciplines.py` and the schema `$defs/discipline` enum are one mirror, `test_disciplines.py` pins it), and anything added later that reads like a lookup table. *(PRD D-L)*
