...
0a. ~~Vector mapping — `maplibre_gl` + PMTiles~~ — **complete (SPIKE-14, 2026-08-15)** — **negative on `maplibre_gl`, positive on the goal.** Neither `maplibre_gl` nor its successor runs on Flutter desktop at all; `flutter_map` + `vector_map_tiles` does, offline, and the basemap source and licence are settled — **ARCH §7.2/§9.2/§11, D22/D23, Q9/Q10, PRD FR95, MVP §1.4.5**
...
**Left open, deliberately:** **only Linux was tested** — Flutter's Windows toolchain is unreachable from WSL. Given that this spike's headline finding is *platform tags lying about desktop support*, "pure Dart, so Windows is probably fine" is exactly the inference it just disproved, so this is recorded as untested rather than assumed. The harness is environment-driven and runs as-is on a Windows Flutter install. The **labels** gap needs a call (Protomaps v3 theme, a reduced Plotlines theme, upstream expression support, or drawing labels as Flutter widgets from the `places` layer). Both are build tasks, not experiments. **Closed.**
