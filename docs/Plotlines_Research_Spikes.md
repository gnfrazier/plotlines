...
1. ~~Multimodal / paddling data availability~~ — **complete (SPIKE-04, 2026-08-14)** — its build-side successor is **SPIKE-19**, which is implementation-informing *unless* the retired-source check fails
...
**Update (2026-08-16): SPIKE-19 added — the desktop MVP is unaffected, but SPIKE-04's answer now has a dated shelf life.** The paddling provider is Leg 3 work (MVP §1.2), so nothing about the MVP build changes and no new spike gates it. What changes is the standing of a closed result: SPIKE-04 remains correct on its own question — the paddling network exists and the gauges are excellent — but the **product** it named, NHDPlus HR, was retired by USGS on 1 October 2023 and is no longer maintained, and the gauge API it measured is decommissioned in Q1 2027. Neither is a failure of that spike; both are the ordinary decay of an external dependency, and the register is where that should be visible before the code is written. **SPIKE-19 should run before the Leg 3 provider is built, not after.**
...
**A sixth spike, SPIKE-19, was added 2026-08-16 — from a working note, and it turned up something the note did not know.** It proposes building the paddling provider against USGS 3DHP flowlines with live gauge binding, which reads like implementation follow-through on SPIKE-04. It is not only that: **USGS retired the National Hydrography Dataset on 1 October 2023 and stopped maintaining NHDPlus HR**, the source SPIKE-04 settled on and that ARCH §11, §13.2 and D19 all rest on. 3DHP is its replacement, not an alternative to it. That makes SPIKE-19 partly a **migration-readiness** spike — and, together with the USGS WaterServices decommissioning already scheduled for Q1 2027, means **both halves of the paddling data answer are mid-migration at once.** The note also carried seven proposals that conflict with decisions taken since it was written — `maplibre_gl` (which SPIKE-14 found does not run on Flutter desktop), an edge normalization proxy (a **P3** design event belonging to SPIKE-17), American Whitewater difficulty data (**prohibited**, D19), and a system-generated "Runnable" verdict (the capability D19 removed). Those are recorded in the entry as things the spike must *not* inherit, which is the same service this document performed for SPIKE-14's `maplibre_gl` assumption.
...
**Carried forward to SPIKE-19 (added 2026-08-16):** this spike's verdict stands, but the *product* behind it does not — **USGS retired the NHD and stopped maintaining NHDPlus HR on 1 October 2023**, with 3DHP as the single replacement, and the gauge API measured in §5 is decommissioned in Q1 2027. SPIKE-19 checks whether the four attributes §3.2 depends on survived the succession, takes this spike's untaken threshold decision (outstanding item 2), and does the geometry pull §10 records as skipped (item 4).
...
**Priority:** Implementation-informing — **and scope-shaping if its first task comes back negative**, because SPIKE-04's answer rests on a dataset that is no longer maintained.

**Why this exists at all: the source SPIKE-04 settled on has been retired.** SPIKE-04 measured **NHDPlus High Resolution** and made it load-bearing — ARCH §11's waterway row, §13.2's `reachcode` rule, and D19's whole scope call sit on it. **USGS retired the NHD on 1 October 2023 and shifted production to the 3D Hydrography Program**; NHDPlus HR remains downloadable but is **no longer maintained**, and 3DHP replaces NHD, WBD and NHDPlus HR with a single product built from lidar-derived elevation. So what this repo currently documents as the paddling network source is a **frozen snapshot**, and the reference note's 3DHP endpoint is not an alternative to it — it is its successor. Put that beside the decommissioning of USGS WaterServices in **Q1 2027** that SPIKE-04 §5 already scheduled, and **both halves of the paddling data answer are mid-migration at the same time.** This spike is migration-readiness as much as feature work, which is why it should run before the Leg 3 provider is written rather than after.
...
The layer metadata for the note's `FeatureServer/1` did not return when this entry was written, which is a reason to verify rather than assume.

**And the identifier join is the sharpest unknown in the spike.** The note proposes binding gauges via `NHDPlusID` / `Permanent_Identifier`; ARCH §13.2 requires `reachcode` on every edge specifically because **NLDI indexes NWIS sites by it**. 3DHP is organised around *mainstem* identifiers. Which identifier actually joins **3DHP ↔ NLDI ↔ NWIS site** is the one question that, answered wrong, silently turns SPIKE-04's strongest result — 58 of 59 gauges resolved to a reach by lookup — back into a proximity match.

**Seven things in the reference note that this spike must not inherit.** The note was written against the product, not against this repo's decisions, and each of these conflicts with something already settled:

| The note proposes | Why it cannot stand as written | What replaces it |
|---|---|---|
| Render flowlines in **`maplibre_gl`** | **SPIKE-14: no MapLibre binding runs on Flutter desktop at all.** The stack is `flutter_map` + `vector_map_tiles` (D22) | A `flutter_map` polyline overlay — and SPIKE-14 already measured route geometry as **free** (0% of frames over budget at 41k vertices), so this task is largely pre-answered rather than open |
| Line simplification and snapping in **Dart isolates** to protect frame rate | Same measurement: the geometry is not the cost, and where client-side heavy work needs an isolate is **SPIKE-15's** question over the whole payload | Drop from this spike's scope; if a threshold is needed it comes from SPIKE-15 |
| A **serverless edge proxy** caching gauge data for 15 minutes | **P3 enumerates five things the hosted service does and normalization is not one**; desktop MVP has no hosted tier (MVP §1.2). This is already **SPIKE-17's third sub-question** | Per-source adapter inside the provider, cached locally under P7. A proxy remains legitimate — as an explicit sixth P3 item taken as a decision, gathered by SPIKE-17, **not smuggled in as a paddling implementation detail** |
| Combine **American Whitewater** reach difficulty into the trip manifest | **D19 / SPIKE-04 §2: reuse is prohibited, there is no API, and no AW data was collected — deliberately.** Automated retrieval is the thing their terms forbid | Nothing. Class stays Author-declared. **Do not scrape** |
| A gauge payload carrying `"status": "Normal / Runnable"` | **"Runnable" is a difficulty verdict** — the capability D19 removed, reappearing as a string. It is also a safety claim the system cannot support from a discharge number | FR14 as narrowed: show the reading, age-stamp it, and state whether it is inside or outside **the Author's** band. The app never grades the water |
| Take stream slope from **3DHP's Z-aware coordinates** | **D20 names GEDTM30 via OpenTopography as the single elevation source with no fallback.** A second source means a river's gradient and the trip's elevation profile can disagree | Not obvious either way — lidar-derived water-surface Z may genuinely beat a 30 m DEM on a river. **Measure both on a known reach and take it as a decision**: either D20 holds, or water-surface elevation becomes a stated exception in the Decision Log |
| A **Flutter prototype that queries 3DHP**, feeding the in-field **Cue Sheet HUD** | Two boundary violations. Data input runs in `plotlines-core` because it must influence `edge_cost` during scoring (ARCH §13.1, P1/D21/FR92) — a client querying USGS directly inverts that. And the position-aware cue sheet is **FR47 / story I1, Leg 5**, gated by SPIKE-06 and SPIKE-12 | The prototype is Python in the core behind `WaterwayDataProvider`; the display surface is the **Author Desktop segment inspector (B8)**, which is where FR14 actually lands |

**What the note gets right and should be kept:** the two USGS endpoints and the parameter codes (`00060` discharge, `00065` stage) are correct and match SPIKE-04 §5; the normalized gauge payload is the right shape once the verdict field is removed; and the **corridor-buffer clip** — restricting an offline waterway pull to a buffer along the active route — is the correct answer to package size, and should be sized here rather than in SPIKE-10, since only this spike knows what a clipped flowline network weighs.
...
| **SPIKE-19 Waterway routing on 3DHP + gauge binding** | FR14/FR14a (**B8**), FR15; ARCH §11, §13.2, D19 | Implementation — **scope-shaping if the attribute check fails** | Possibly — **SPIKE-04's source was retired.** USGS stopped maintaining NHDPlus HR on 1 Oct 2023; 3DHP replaces it. Whether the successor still carries `fromnode`/`tonode`, `flowdir`, `streamorde` and `reachcode` is unverified, and each has a known failure mode. Also takes SPIKE-04's untaken threshold decision and produces the geometry-based route it never computed |
