...
> **Priority split within this flow.** The **left branch is MVP**: layers → candidates → promote directly → roles → anchor. The **right branch (N4, N4a) is [P1]** — cluster analysis and proposal review are assistance layered on top of a workspace that must already work without them. Design may mock the P1 branch early; **it must not be a prerequisite for the MVP path**, and an implementation in which promotion is only reachable through a proposal has inverted the dependency.

| Node | Priority | FR | Story | Note |
|---|---|---|---|---|
| Select live layers | **MVP** | FR97, FR100, FR101 | N3, N5 | Defaults vary by **mode × day-type** and are config, not code. *(N5, plugin layers, is [P1].)* |
| Candidates on map | **MVP** | FR98, FR99 | N3 | Salience is a score, not a verdict. Sub-weighted `historic=*`. |
| Know the area already? | **MVP** | — | N3, §5 | **Skippability is the rule, not a shortcut.** Every stage below this is assistance. |
| Find the good spots | **[P1]** | FR102, FR105a | N4 | Named Author action over the bbox. **Never ambient on viewport change.** |
| Review proposals | **[P1]** | FR103–FR105a | N4a | Cards, contributing features with salience, bulk reject, rejections remembered. |
| Assign role set | **MVP** | FR105, FR106, FR110 | O1 | Roles proposed by **affinity union** where a proposal exists; always editable. Reachable directly from a candidate at MVP. |
| Anchor in the trip | **MVP** | FR106 | O1 | Copies from its candidate — **never references it** (survives a cache wipe). |
| Anchors view | **MVP** | FR139, FR142(b) | Q2, K12, N4a | **The workspace has three views, not two:** candidates, proposals, anchors. The **proposals view is [P1]**; candidates and anchors are MVP. Unattached anchors (flow 9) are **ordinary working state, not a problem queue** — not badged, not counted as errors, blocking nothing. |
...
flowchart TD
    ...
    I --> K[Proceeds]
    A -.->|or duplicate instead| L["Duplicate with edits<br/><i>content yes, position no</i>"]
```
...
| Duplicate with edits **[P1]** | FR141 | Q4 | **An alternative to editing, not a step in it** — hence the dashed branch. Carries content and settings; **carries neither provenance nor position**, since a duplicate at the original's coordinates is two anchors in one place, exactly what the anchor model exists to prevent. For a nine-day tour with a repeating rhythm this is an afternoon versus an evening. |
...
**Deliberately not drawn:** D8 (roster board, FR137) and D9 (travel circles, FR143) are *[Later]*. The MVP path for a recurring group is flow 1's clone branch.
