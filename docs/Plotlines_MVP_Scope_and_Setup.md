...
**This file is the current reference for the desktop-MVP story list (§1.4)'s five screens** — `00 New Route`, `01 Route Planner`, `02 Constraint Conflict`, `03 Node & Narrative`, `04 Cue Sheet + Export` — plus a trips library and settings screen the 2026-08-17 re-export added. The `client/lib/presentation/` build described in §8 was originally built against an *earlier* pass of this wireframe and extended past it where that pass fell short of §1.4's 32 stories (via-node UI, band controls, a real shape selector, rest days/regroup, attribution). **Reconciled against the 2026-08-17 revision on 2026-08-17** (a same-day follow-up pass): the client now uses the wireframe's persistent tabbed shell per trip (`Route / Logistics / Content / Export`) instead of separate pushed screens, with an always-visible weights rail, a day timeline strip, and A6's conflict flow as a real modal dialog — see `client/design/README.md` for the one deliberate deviation (Logistics has no wireframe mockup to reconcile against; its content is this repo's own design) and the export-tab content-toggle/splitting features that came with it.
