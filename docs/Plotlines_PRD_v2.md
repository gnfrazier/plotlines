...
| **FR144** | **[NEW v2.0; #315]** A trip **declares one or more travel-mode categories at initiation** (#315 — the broad category, not a specific discipline), alongside its title, location prompt, and bounding box. **At least one is required.** The declared set is the input FR97's layer defaults have been missing — it seeds which layers are live and which categories are offered at passage creation (FR10); a discipline under a category is a per-passage choice, not declared here. It is **not a constraint**: an Author may create a passage in an undeclared category, and doing so **adds it to the trip** without warning or block. Categories are editable for the life of the trip; changing the set updates layer defaults for days the Author has not overridden and **leaves overridden days alone**. A cloned trip carries its source's categories (FR74). *Without this, FR97's central mechanism — "defaults vary by (travel category × day type)" — has no argument to work from, and the layer picker has to assume cycling.* | Design cross-check CR-1; #315 |
...
**N0 — Declare how we're travelling** *[MVP]* **[NEW v2.0; #315]** — *FR144*
...
*AC:* A trip **declares one or more travel-mode categories at initiation** — #315: the broad category (Cycle · Foot · Paddle · Ski · Drive), offered as equal targets with no "common vs every other mode" tiering and no `transit` (it is a note mode, never in a "how will you travel" list); a specific **discipline** under a category is a per-passage choice, not declared here — alongside its title, location prompt, and bounding box, and the declaration step **precedes the location prompt**; **at least one is required**. The declared set is the argument N3's layer defaults have been missing — it seeds which layers are live and which categories are offered at passage creation (B1), and **the layer picker states which categories it derived its initial state from**. **Declaring is not constraining**: an Author may create a passage in an undeclared category, and doing so **adds it to the trip** without warning, block, or confirmation. Categories are **editable for the life of the trip**; changing the set updates layer defaults for days the Author has not overridden and **leaves overridden days alone**. A cloned trip carries its source's categories (G2), and a roster-only clone, having nothing to inherit, runs trip initiation normally (G2b, N1). **Climbing, canyoneering, and jumaring are stations, not travel modes** (O4) and never appear in a mode list.
