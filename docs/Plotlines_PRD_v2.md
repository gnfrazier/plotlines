...
**Acquisition runs behind the Author, and nothing waits behind anything else.** Readiness is **per-capability, not global**, and the capabilities are independent: layer extraction and POI indexing unlock authoring, the region graph unlocks routing, elevation enrichment unlocks only elevation-dependent metrics. All three start at once, and the Author is productive throughout — selecting layers, reviewing clusters, promoting anchors — with an honest, non-nagging indicator on whatever is not ready yet. *(FR121 — this changes FR91 and ARCH §7.3 rather than extending them. **Prior reading: "enrichment is a blocking, minutes-long operation"** and routing unlocks when it finishes — measured 2026-08-28 at **8.8 s**, against extraction's 15.8–178.5 s and a region graph build of 36.7–116.6 s, which is what routing actually waits on.)*
...
*(FR105a, N4a. The rendering strategy for candidates and proposals at bbox scale is settled — ARCH Q15, SPIKE-G: salience-gated markers (the notable candidates render as hit-testable pins, the rest as a salience-styled dot field), with grid clustering below the trip-overview zoom and above a ~2,800-candidate display ceiling.)*
...
| **FR144** | **[NEW v2.0; #315; AMENDED #319]** A trip **declares one or more travel-mode categories at initiation** (#315 — the broad category, not a specific discipline), alongside its title, location prompt, and bounding box. **At least one is required.** The declared set is **the trip's one mode set** (#319, ARCH D64): the input FR97's layer defaults have been missing — it seeds which layers are live — and **the only categories a passage may be** (FR10): the per-passage picker offers exactly this set, and adding a category to the trip is a trip-level action available from the planning page beside that picker, so "I need Drive for one passage" is *add Drive to the trip, then pick it*. A passage that arrives in an undeclared category by another path (a clone, an older payload) **adds it to the trip** without warning or block. The set **never shrinks on its own** — removing the last passage in a category leaves the category on the trip; only the Author removes one. Categories are editable for the life of the trip; changing the set updates layer defaults for days the Author has not overridden and **leaves overridden days alone**. A cloned trip carries its source's categories (FR74). *Prior reading (#315): "not a constraint" — an unfiltered passage picker that silently added to the trip; #319 replaced it with the filtered pick because two selectors offering different lists read as a duplicate and let an Author believe the passage pick had changed the trip.* *Without this, FR97's central mechanism — "defaults vary by (travel category × day type)" — has no argument to work from, and the layer picker has to assume cycling.* | Design cross-check CR-1; #315; #319 |
| **FR121** | **[NEW v2.0; AMENDED 2026-08-28 on SPIKE-D]** Readiness is **per-capability, not global**, and the capabilities are **independent rather than ordered**. **Layer extraction and POI indexing** unlock the authoring surfaces (layer selection, candidate display, co-location analysis, promotion). **The region graph** unlocks routing. **Elevation enrichment** unlocks only elevation-dependent metrics. All three start at trip initiation and each reports its own state; **none waits behind another**. Controls requiring a capability that is not ready are visibly disabled with a stated reason and an honest progress indication, never silently failing on click and never blocking the app. **The indication is a range or observed progress, never a fixed figure** — acquisition runs **×2.96 slower while the Author is working**, so a constant estimate is wrong precisely when the Author is busiest. **Prior reading (v2.0): layer extraction "completes first" and elevation enrichment "unlocks routing and elevation-dependent metrics on completion", with an indicator reading "terrain data loading — routing available in about 3 minutes".** Measured false on both counts: elevation enrichment is **8.8 s** against extraction's **15.8–178.5 s**, and routing waits on the **region graph** (36.7 s at 704 km², 116.6 s at 1,799 km²). A reader of the prior wording would wire routing readiness to the wrong capability. | Decision D-E; amended on SPIKE-D |
| **FR91** | **[AMENDED v2.0; AMENDED AGAIN 2026-08-28 on SPIKE-D]** Region acquisition at trip initiation **must run off the request-handling event loop**. **Prior reading (v1.0): a sidecar still enriching elevation reports itself *not ready*, gating the whole app** — superseded by FR121: the sidecar reports **readiness per capability** and the client enables surfaces accordingly; a single global ready/not-ready flag is not sufficient. **Prior reading (v2.0): elevation enrichment is a "blocking, minutes-long" operation.** Measured **8.8 s** over a 704 km² trip bbox — of which node sampling and grade computation are **110 ms**, the rest being DEM acquisition. Nothing in this repository had ever measured that claim. **The operation that *is* minutes-long is the region graph build** (36.7 s at 704 km², 116.6 s at 1,799 km²), and it is what routing waits on; the off-the-event-loop requirement stands and attaches to that first. **Extraction is the other long pole and the unpredictable one** (15.8–178.5 s for the same query). *(Requires a corresponding change to ARCH §7.3 health semantics.)* | POC; ARCH §7.3, amended; SPIKE-D |
...
| **FR99** | **[NEW v2.0]** Candidates render on the planning map as **toggleable layers with salience visible** (size, weight, or opacity). An Author can promote a candidate directly from the map and complete an entire trip without invoking co-location analysis — every later pipeline stage is assistance, never a required wizard. **Rendering strategy (ARCH Q15 / SPIKE-G): salience-gated** — the top candidates by salience in view render as hit-testable markers, the remainder as a salience-styled dot field, with grid clustering below the trip-overview zoom and above a ~2,800-candidate display-density ceiling. Promotion and list↔map two-way selection operate on the marker set directly. | Pipeline §5.3; ARCH Q15 |
...
| **FR116** | **[NEW v2.0]** **Print and web inherit reveal policy.** A printed cue sheet or itinerary shows every provision, every hazard, and the arc's shape — but not the content of unrevealed plot points. **The paper copy cannot spoil the trip.** For the **web reading surface**, an accountless reader is resolved as a permanently empty revealed set (SPIKE-F / ARCH D59): a held plot point renders as a withheld placeholder that keeps its arc stage but no content, so the arc's shape survives while the content does not. | Decision D-H; SPIKE-F |
...
| FR | Requirement | Origin |
|---|---|---|
| **FR10** | **[AMENDED v2.0; #315]** Authors create **passages** with a start, end, and primary **traversal category**, optionally refined by a **discipline** under it. Plotlines distinguishes **traversal categories** — cycling, hiking, paddling, cross-country skiing (Ski), and **driving** — from **station activities** (FR109), which are performed at a place rather than between two. **#315 settled Model B:** a discipline (road / gravel / mountain, foot's hike / run / trail run, and so on) is a *variant under a category* that selects a `WeightProfile`, never a category of its own; `mountain_biking`, `packrafting` and `riverboarding` were categories through v2.0 and are now the `mountain` / `packraft` / `riverboard` disciplines (`multimodal/legacy.py` maps the old spelling forward). Categories are declared at the trip level (FR144); a discipline is chosen per passage. **Prior reading (v1.0): three first-class modes with "further modes" deferred as a scoping decision** — which quietly filed climbing, canyoneering, and jumaring as travel, though `WeightProfile` models only horizontal traversal and could never have absorbed them. | Plotlines Story 2; corrected v2.0; #315 |
| **FR130** | **[NEW v2.0; #315]** Adding a **traversal category** requires only a new `WeightProfile` entry and its domain parameters (M1) — no parallel scorer. Adding a **discipline** under a category is the same shape: a `WeightProfile` entry, no scorer branch (`multimodal/disciplines.py`). Adding a **station activity** requires only an activity-type entry — no routing change at all. The extension paths are distinct and all are configuration. | M1 seam; Decision from §4.3; #315 |
...
| **FR14b** | **[NEW v2.0; AMENDED 2026-08-28 after SPIKE-C]** Where a source publishes difficulty grading, Plotlines reads it — but **what it may do with what it reads depends on the schema, because coverage was measured and only one of the four supports a summary.** Three clauses. **(a) Nordic reads and aggregates.** `piste:difficulty` is read from source per way and rolled up to a leg; `freeride`/`extreme` are handled as off-scale, not as ordinal extremes above `expert`. **(b) Hiking and MTB read *per way* and never aggregate.** Where a way carries `sac_scale`, `trail_visibility`, `mtb:scale`, `mtb:scale:uphill` or `mtb:scale:imba`, show it on that way, attributed to its source. Do **not** roll it into a leg or day grade: at measured North American coverage a worst-of summary is a **16–32% chance of a confident understatement**, always in the easy direction. Both MTB scales are read and neither is converted into the other — they are regional mirror images (`:imba` 39.3% in Bentonville / 0.10% in the Tyrol; `mtb:scale` the reverse). **(c) Author declaration (FR14) is the *primary* source for leg-level land difficulty**, not a fallback — SPIKE-C inverts v2.0's original wording here. Source and coverage are stated on every reading (see B9). **SPIKE-04's whitewater finding did not generalize, and neither did the hope that replaced it.** | OSM attribute mapping; **resolved by SPIKE-C (#170), ARCH Q13** |
...
| **FR29a** | **[NEW v2.0]** A driving leg carries a **vehicle-access advisory**. The Author declares an expected vehicle capability (2WD / AWD / high-clearance / 4WD); Plotlines reads the road's own signals — `surface=*` (gravel, dirt, unpaved, compacted, ground), `smoothness=*` (`very_bad` through `impassable`), `tracktype=grade1`–`grade5`, `4wd_only=yes`, `highway=track`, `motor_vehicle=*` — and **flags where the route exceeds the declared capability**, in the leg summary and on the cue sheet. **Advisory, not a constraint** — it warns and surfaces, it never excludes or reroutes, on the same footing as FR14's gauge band. Two reasons: tag coverage is uneven, and whether a given vehicle will make a given road is the Author's judgment, not the app's. **Tag coverage is stated plainly** rather than implying completeness, per the FR46 honesty clause. This is the requirement behind *"sometimes the last mile to the trailhead or put-in is the most harrowing moment of the day."* **[NARROWED 2026-08-31 — SPIKE-E (#171), measured over four real approaches and 12,069 ways / 2,419 km.]** Four clauses the signal list above does not imply: **(a)** *"stated plainly"* has a threshold — the **"no contrary signal found" sentence requires ≥70% of the leg's kilometres to carry a condition tag**, and below that the only permitted statement is that the route is unsurveyed, in wording that shares no claim with the clear case. At a 20% floor a real 35%-surveyed gravel approach reported "no signal exceeds 2WD"; the seeded thinning model puts that confidently-wrong outcome at 18.8% against a pre-declared 10% ceiling, and at 0.0% at 70%. **(b)** Coverage is computed and stated **per last mile as well as per leg** — the dangerous shape is a long well-surveyed paved drive with a short unsurveyed rough tail, which is this requirement's own sentence; one approach measured 51.3% over the leg and **0.6% over its last 5 km**. **(c)** `4wd_only` is an **opportunistic read, not a required signal** — 0 tagged ways in the whole measurement, two National Forests included — and no coverage statement counts it; `smoothness` is absent at network scale (0.3–4.3%) and present on 100% of the worst approach's last 3.4 km, so it is read per way and never as a network figure. **(d)** `motor_vehicle` produces an **access note, not a capability flag**: it says who may drive, not what they need to drive in, and an Author cannot act on it by borrowing a better vehicle. | Concept framing; OSM road-surface tags; SPIKE-E |
...
| **FR45** | Exported waypoints, regroup markers, rest-stop names, and **plot-point notes** are preserved as native course/turn points where the target format supports them. *(In v2.0 "plot point" is a defined object — see §4.3 — not an undefined phrase.)* **For FIT (SPIKE-16): the format has a native `course_point` slot for every marker type Plotlines produces — turns, `water`, `food`, `danger`, `generic`, `rest_area`, summit/valley — so "where supported" is all of them at the format level; which head unit actually draws each type is the per-device table in the spike's `HARNESS.md`. Note text rides in `course_point.name` (no separate description field) and is word-boundary truncated at the measured device floor. Area anchors (FR108) have no FIT representation and are GPX/GeoJSON-only in export unless the harness shows a centroid point reads better; role offsets (FR107) export at their offset position.** | CTP FR9, C14 |
...
| **FR78a** | **[AMENDED v2.0]** An Author **requests the specific profile fields** they need for a trip, forming the request a Character responds to. **The request set now includes non-profile permissions of the same shape — notably arrival visibility (FR123)** — so that consent for in-field sharing uses this one surface rather than a parallel mechanism. The request is a default set the Author can adjust per trip; it never auto-grants access. **An Author may also record, per Character, a value for any field in the request set that they already hold from outside the app** — the roster that arrived by text, email, and conversation weeks before anyone opened Plotlines. Author-entered values carry **provenance as a first-class discriminator** (`entered by the Author`, distinct from `granted` and `volunteered`), are shown visibly distinct wherever the field appears, and **never render as `granted` and never satisfy the request**, which stays outstanding until the Character responds. **Recording a value opens no sharing surface**: an Author-entered field is visible to that Author alone and never enters a roster-visible surface a granted value would, arrival visibility (FR123) included. The Author may **edit or clear their own entry** at any time, and doing so changes neither its provenance nor the outstanding request. A Character's later response through K2 **supersedes** the Author's entry rather than merging with it — the Character can see that an Author-entered value existed and may **edit, update, or amend the value that then stands** — and the field's audit shows both. Author-entered values are information one person holds about another, so they fall under **FR135a / D6a deletion on request** exactly as Author notes do, inheriting that deletion path rather than a new one, and a **clone that carries the roster carries them** (FR74/FR74b) — they are the Author's own record, not a grant, so the profile-grant clone exclusion does not reach them. | New; extended v2.0 & D4b |
...
| **FR138** | **[NEW v2.0]** Plotlines carries a **plain-language privacy statement, reachable from the About surface on every platform** — including the lightest ones (Web guest, and the share-token reading view, FR132). It states plainly: what is stored on the device and what reaches the server; that **reveal is a product guarantee against accidental spoiling and not a security boundary** (FR64a); what arrival sharing does and does not do, and that it defaults to nothing shared (FR123); that **an Author may keep private notes about Characters, that those notes are visible only to that Author, persist across trips, and can be deleted on request** (FR135, FR135a); and that guest sessions leave no server-side trace. It also states, for the **share-token reading view**, what a reader with no account leaves behind (SPIKE-F / ARCH D59): a short-lived operational log line with no account, no name, and a truncated IP — edge/CDN logs kept no more than 72 hours and application logs no more than 30 days — and that the share link carries no readable token after the first load. **Author notes are the first data in Plotlines held *about* a person *by someone other than that person***, which is what makes this statement an obligation rather than a courtesy. Not legal boilerplate — it says what is true, briefly. | ARCH §13.4; FR135 consequence; SPIKE-F |
...
| **FR88** | **[AMENDED 2026-09-24, #473 / ARCH D68]** Elevation reads **never raise and never block a route solve**, and distinguish a **gap** from an **absence**. A gap inside a raster that opened — `nodata` (including raw NaN, checked via `isnan`), `inf`, or a coordinate outside the raster's bounds — is **linearly interpolated from the nearest finite samples** (by distance along a route; from graph neighbours during enrichment), falling back to `0.0` only when no finite sample exists anywhere to interpolate from. A missing/unreadable raster, or a bbox no source resolved, is **absent** — no profile, no elevation attribute — never a fabricated flat `0.0`. Each void logged **at most once per raster path**. No network fetch inside route computation. **Prior reading: every void, gap or absence, resolved to `0.0` ("not configurable"), so a handful of nodata pixels on good coverage reported a plunge to sea level as ascent and descent, and a region with no DEM reported a measured-looking flat profile.** | POC; SPIKE-18; #473 |
| **FR89** | **[AMENDED 2026-09-24, #473]** Elevation enrichment annotates every graph node with its elevation and every edge with `elev_gain = max(0.0, elev[v] - elev[u])` — positive gain only — **whenever an elevation source resolved**. With none, it annotates nothing (FR88's absent case) and strips any earlier annotation; readers treat a missing attribute as "no elevation". Enrichment never raises. **Prior reading: every node always annotated, flat `0.0` with no raster.** | POC; SPIKE-18; #473 |
...
**N0 — Declare how we're travelling** *[MVP]* **[NEW v2.0; #315; AMENDED #319]** — *FR144*
**As an** Author, **I want to** declare the trip's travel-mode categories when I create it **so that** the layer picker and passage creation start from how we are actually travelling rather than assuming cycling.
*AC:* A trip **declares one or more travel-mode categories at initiation** — #315: the broad category (Cycle · Foot · Paddle · Ski · Drive), offered as equal targets with no "common vs every other mode" tiering and no `transit` (it is a note mode, never in a "how will you travel" list); a specific **discipline** under a category is a per-passage choice, not declared here — alongside its title, location prompt, and bounding box, and the declaration step **precedes the location prompt**; **at least one is required**. The declared set is the argument N3's layer defaults have been missing — it seeds which layers are live and which categories are offered at passage creation (B1), and **the layer picker states which categories it derived its initial state from**. **The declared set is the trip's one mode set** (#319): the passage picker offers only these categories — with *add a category to the trip* on the same planning page, so a category the trip lacks is one gesture away — and a passage that arrives in an undeclared category by another path (clone, older payload) **adds it to the trip** without warning, block, or confirmation; the set never shrinks on its own. **No category is preselected for a new passage**; a change of category marks a solved passage stale, a change of discipline within it does not. Categories are **editable for the life of the trip**; changing the set updates layer defaults for days the Author has not overridden and **leaves overridden days alone**. A cloned trip carries its source's categories (G2), and a roster-only clone, having nothing to inherit, runs trip initiation normally (G2b, N1). **Climbing, canyoneering, and jumaring are stations, not travel modes** (O4) and never appear in a mode list.
...
*AC:* Layer extraction and POI indexing unlock layer selection (N3), candidate display (N3), co-location analysis (N4), and promotion (O1); **the region graph unlocks routing, and elevation enrichment unlocks only elevation-dependent metrics** — three independent gates, started together, none waiting behind another (FR121); a control needing a capability that is not ready is visibly disabled with a stated reason and **a progress estimate given as a range or observed progress, never a fixed figure** (acquisition runs ×2.96 slower while the Author works); no control silently fails on click; the app is never globally blocked; **`layers.ready` is `any`, not `all`** — an `all()` reinstates the global flag this story exists to remove. **Plugin layer loading follows the same non-blocking rule and is tracked per layer, not as one flag**: a slow or remote plugin dataset (N5) shows as *loading* in the picker while built-in layers are already usable, **one plugin layer failing to load never blocks the others or the workspace**, and a failure states which layer and why. **Satisfying that last clause requires the curation provider layer to match ARCH §14.2** — a per-layer `fetch_candidates(bbox)` and a `load_state()`, rather than the shipped single multi-layer `fetch(bbox, layers) -> list[RawFeature]` — **and requires `GET /candidates` to return the layers it did serve alongside the ones it could not, instead of failing the whole request.** *(Measured 2026-08-28, SPIKE-D: six built-in layers plus one failing plugin returns **HTTP 422 and zero candidates**; the shipped app passes 3 of this AC's 8 clauses. Prior AC wording said extraction "completes first" and that routing unlocks when enrichment completes — both corrected above.)* *(Requires per-capability readiness — with per-layer state inside the layers capability — in the sidecar `/health` contract; ARCH §8.3 change, see M12a.)*
...
*(Design note: PRD §5.4a carries a worked walkthrough against a named region. Candidate and proposal rendering at bbox scale is settled — ARCH Q15, resolved 2026-08-29 by SPIKE-G: salience-gated markers + a canvas-dot tail + a grid-cluster backstop, ~2,800-candidate GPU display-density ceiling, A16 restated at ~1.15 GB.)*
...
*AC:* **[NARROWED 2026-08-28 by SPIKE-C (#170) — coverage was measured, and the AC is now the schemas that cleared it.]** `piste:difficulty` is read and summarised for nordic legs. `sac_scale`, `trail_visibility`, `mtb:scale`, `:uphill` and `:imba` are shown **on the way that carries them and never rolled up** — a leg or day grade for hiking or MTB comes from the Author (B8), which is the source rather than the fallback. Every reading carries its source and its coverage: three states, and the sparse one cannot render as the complete one — *graded* ("published for every way on this leg"), *partial* ("X is the hardest grade among 3 of 7 graded ways, 9.6 of 20.5 km — this is a floor, not a summary"), *silent* ("No published grade for this leg — the grade is the Author's to declare, not the map's to supply"). Silent must never render as easy. Unrecognised values (`sac_scale=strolling`, `mtb:scale:imba=2.5`) are reported as ungraded-but-present, not discarded. Per-region coverage figures ship in the shape the UI consumes (`spikes/SPIKE-C/results/coverage.json`).
...
*AC:* **Driving legs are generated by the routing engine in driving mode**, producing a real route with distance, time, and a cue sheet, exportable like any other; **train, shuttle, and flight legs are authored notes** carrying identifiers, carrier, scheduled times, and links; both attach to trip start/end or a day and render in itineraries; neither is a live-status or booking integration. *(v1.0 flattened all four into authored notes and left driving out of the mode list.)* **[MEASURED 2026-08-31 — SPIKE-E (#171).]** The routing clause holds — the shipped solver routes real trailhead and put-in approaches in 2.8–14.5 ms — with two blockers named rather than assumed away. **The graph the mode is configured against cannot see the last mile**: `network_type="drive"` is an OSMnx download filter that excludes `highway=track` and `highway=service`, so a `service`-tagged trailhead road produced a route ending **265 m short, reported as success**. **And the "time" in this AC is not yet real**: a flat 60 km/h mode speed is 44.6% short on a 71.8 km dirt approach and 19.7% long on a paved one, so this clause is satisfied on distance and cue sheet and **blocked on B7/FR16** for time.
...
*AC:* Author declares expected vehicle capability per driving leg (2WD / AWD / high-clearance / 4WD); Plotlines reads `surface=*`, `smoothness=*`, `tracktype=*`, `4wd_only=*`, `highway=track`, and `motor_vehicle=*` and **flags the sections that exceed the declared capability**, in the leg summary and on the cue sheet, with the specific signal that triggered the flag. **Advisory only — it warns, it never excludes or reroutes**, on the same footing as B8's gauge band: tag coverage is uneven, and whether a given vehicle makes a given road is the Author's call. **Coverage is stated plainly** — an unflagged leg means *no contrary signal found*, not *confirmed passable* — per the honesty clause in F1. **[NARROWED 2026-08-31 — SPIKE-E (#171).]** *"No contrary signal found"* may only be printed at **≥70% surveyed kilometres**; below that the leg reports itself unsurveyed. Coverage is reported for the **last mile separately**. **Sections are merged, never per-edge** — condition tagging is fragmented along one road, and per-edge flagging produced 65 sections for a single 42.6 km dirt approach; runs merge at the same requirement across unsurveyed gaps up to 500 m and never across a stretch tagged clear. `4wd_only` is read opportunistically and never counted as coverage (0 tagged ways measured); `motor_vehicle` renders as an access note rather than a capability flag. The advisory cue is its own kind — **not a hazard**: a hazard is unconditional and unsuppressible (FR115) while this is a comparison against a declaration the Author can change.
...
**D4b — Fill in what a Character already told me** *[MVP]* **[NEW v2.0]** — *FR78a*
**As an** Author, **I want to** record the profile fields a Character has already given me through other channels **so that** I am not blocked waiting on a request for information I already hold.
*AC:* Author enters a value for any field in the trip's request set, per Character, **without sending or resolving the request**; every value records and displays its **provenance** — Author-entered / granted / volunteered — and an Author-entered value **never renders as granted and never satisfies a pending request**; the Author can **edit or clear a value they entered** at any time without changing its provenance; a Character's later grant (K2) **supersedes** the entry, the Character can **edit / update / amend the value that then stands**, and the field's audit shows both; Author-entered values are **visible to that Author alone** and enter no roster-visible surface a granted value would (P3); they fall under **D6a**'s delete-what-I-hold path.
...
*AC:* GPX/TCX/FIT; toggle track+elevation, waypoints/stops, cue sheet, variants; single or per-day files; native course/turn points and plot-point notes preserved where supported. **SPIKE-16 settled the FIT writer: it ships in `plotlines-core` in dependency-free Python (~90 lines), not Dart FFI against the Garmin FIT SDK — the FFI arm showed no fidelity gain and would fork one format off the shared core code path, add a native per-platform dependency (risk A5), and pull in the SDK's redistribution/notice obligation. The writer's CRC-16 and container framing are validated against 10 real Garmin activity files (10/10) and a generated course round-trips to the byte across the five required `course_point` types.** **Reveal is applied before the first byte is written** — an unrevealed plot-point note never reaches the output (punch-list §6A.2), while hazard points always export (§1.5). **Two F3 build tasks the spike names:** `course_point.name` carries the note (FIT has no separate description field) and must be cut on a word boundary at the device cue-list floor measured in SPIKE-16's `HARNESS.md`, not hard-truncated mid-word at 64 bytes; area anchors (FR108) have no FIT slot and are GPX/GeoJSON-only in export unless the harness shows a centroid `generic` point reads better on-device. Per-device FR45 fidelity — which head unit draws which course-point type — is the `HARNESS.md` device-run table, which needs hardware this spike did not have; the Python-in-core verdict is falsifiable only by the FFI arm rendering a type or note the Python arm cannot.

**F4 — Review and edit device-limited cue entries** *[P1]* **[NEW v2.0]** — *FR45, FR46*
**As an** Author, **I want to** review each cue entry as it will appear on a character-limited surface and rewrite the short form myself **so that** the entry keeps the context I need it to carry on the Character's device instead of being cut to whatever fits.
*AC:* Wherever a target surface imposes a character budget on a **derived** cue or course-point entry — FIT/TCX `course_point.name` at the head-unit cue-list floor measured in SPIKE-16's `HARNESS.md` (~15–30 chars on Garmin; the format carries no separate description field), the glanceable HUD's focused line (FR52) — the Author can open the entry, see it rendered as that surface will show it, and author a **short form that overrides the automatic truncation**. The automatic derivation from F3 stays the default — word-boundary cut at the measured floor, with a `cp_type`-implied prefix rather than repeating the anchor name — and an entry the Author does not touch ships that derived form; the Author edits only what they choose. The review surface shows, per entry, the **full authored note, the derived short form, the budget in characters, and a live count**; over-budget text is flagged and never silently cut. A short form is **authored content**: per entry, carried in the trip payload, surviving a cache wipe and a re-solve, and outside FR140 staleness. **Reveal is unchanged** — an unrevealed plot-point note has no entry to edit (it never leaves the Reveal Resolver), a hazard entry is always present and always editable but can never be suppressed (O5, §1.5). Editable from both the export-contents surface (F3) and the cue sheet (F1), and the one short form feeds **every** character-limited surface so it is authored once. Per-format budgets are **config, not code**; the budget shown is the conservative floor, the per-device `HARNESS.md` table is advisory. The full cue sheet (F1) and itineraries (F2) are untouched — they keep rendering the complete authored note in the narrative register (FR133).
...
> **Gate — anonymity of the web reading surface. Settled 2026-08-30 by SPIKE-F (#175); recorded as ARCH D59, closing Q17.** An anonymous share-token reader sees **the always-visible set only**; revealed content requires an account. The token is **exchanged once for an opaque `__Host-` `HttpOnly; Secure; SameSite=Strict` cookie** and never left in the URL (SPIKE-F measured a path- or query-carried token leaking into the access log and every subresource's `Referer`; a URL fragment stays off the wire but not out of browser history or a copied link); reading-view responses carry `Referrer-Policy: no-referrer`. Logs for an accountless reader are two-tier — edge/CDN ≤72 h, application logs written through a field allowlist (route template, no token/`Referer`/cookie, IP truncated to /24) ≤30 d — and FR138's privacy statement says so. Reveal has no gap: the reader **is the empty revealed set, permanently**, the projection takes no identity argument, and a held plot point renders as a withheld placeholder that keeps its arc stage but no content. **Still gated to the web/hosted leg (Leg 4); not an MVP blocker.** See ARCH D59 / Q17 and `spikes/SPIKE-F/results/RESULTS.md`.
...
*AC:* `/health` reports readiness independently for tile/basemap, layer-and-POI, routing, and elevation capabilities, **with no capability's state derived from another's**; **routing readiness is per region**, keyed by the id `POST /regions` returned and empty until an Author has drawn a bbox; **layer readiness carries per-layer state** inside the layers capability, and `layers.ready` is `any`, not `all`; anything still warming reports **a range or observed progress, not a fixed constant** (acquisition runs ×2.96 slower while the Author works); the client enables and disables surfaces from those flags and states the reason on any disabled control; M12's version-mismatch refusal is unchanged. **This replaces v1.0's single ready/not-ready semantics and is a breaking change to ARCH §7.3.** *(Prior AC clause: "layer extraction and POI indexing are ordered ahead of elevation enrichment … the Author needs the former immediately and the latter only when routing" — removed 2026-08-28 on SPIKE-D. The capabilities are independent, not ordered; elevation is the cheapest of the three at 8.8 s, and routing waits on the region graph, not on elevation.* **The per-capability and per-region halves are built (#154); the per-layer half is not, and belongs to N2.**)
...
- ~~**Cluster proposal ranking at scale.** FR105a bounds and ranks proposals, but the actual ranking function — how salience and tightness trade off, whether corridor proximity should dominate once a route exists, what "reviewable count" means for a 200 km bbox — is a design and tuning question that wants a spike against real regions.~~ — **measured 2026-08-27 (SPIKE-B, ARCH §18 Q12).** Default rank = combined salience × tightness. **Corridor proximity does *not* dominate** — it is a bulk filter axis and an opt-in resort, because folding it into the score buried genuinely major off-route sights (measured against §5.4a's worked pass). **Reviewable count** = `30 + 0.5 per route-km`, with the count beyond always shown, never truncated. Cost is a non-issue (~150 ms for a real 8,800 km2 six-layer bbox). Shipped in `core/plotlines_core/curation/colocate.py`. One residual, handed back to notability: ranking quality is capped by flat `tourism=viewpoint` / `natural=peak` weights in mountain terrain — SPIKE-A's deferred prominence sub-scaling is the next lever. See `spikes/SPIKE-B/results/RESULTS.md`.
- **`waterway=waterfall` is outside the layer catalog.** SPIKE-B found §5.4a's canonical "castle beside a waterfall" narrative cluster is only computable when the waterfall also carries `tourism=attraction` (Linville Falls does; most OSM waterfalls carry only `waterway=waterfall`, a key FR97's six families do not include). Non-blocking, same shape as SPIKE-A's `natural=water` residual — a mapping pass, not a spike.
- ~~**Anonymity of the Character-facing web reading surface.** FR132/H13 serve an authored journey to a reader with no account. Three unresolved strands: a share token in a URL leaks through referrers, history, and server logs; access and CDN logs are a retention surface; and **reveal state has nowhere to live for an anonymous reader** — no account, no device GPS, nothing to fire a reveal.~~ — **resolved 2026-08-30 (SPIKE-F #175, ARCH Q17 → D59).** Anonymous reader = always-visible set only; revealed content requires an account. Token **exchanged once for an opaque `__Host-` `HttpOnly; Secure; SameSite=Strict` cookie**, never left in the URL (path/query carriers measured leaking it into the access log and every subresource `Referer`; a fragment stays off the wire but not out of history/clipboard), `Referrer-Policy: no-referrer` on the reading view. Logs two-tier — edge/CDN ≤72 h, allowlisted app logs ≤30 d — stated in FR138's privacy statement. Reveal gap closes with no new machinery: the reader is the empty revealed set permanently and the projection takes no identity argument, so there is no trusted-reader spoiler path; held plot points render as a placeholder that keeps the arc stage. Still gated to the web/hosted leg. See `spikes/SPIKE-F/results/RESULTS.md`.
- ~~**Rendering candidates and proposals at bbox scale.** A dense trip bbox may put thousands of markers over a basemap that already costs ~1 GB. Clustering-for-display, zoom thresholds, or salience-gated rendering — undecided, and its answer governs the map half of N4a. *(ARCH Q15.)*~~ — **resolved 2026-08-29 (SPIKE-G, #161): salience-gated markers + canvas-dot tail + grid-cluster backstop; ~2,800-candidate GPU display-density ceiling; A16 restated at ~1.15 GB (candidate layer immaterial, tile-cache bounds the lever). Recommendation on a model calibrated to SPIKE-14; two-platform Flutter re-measurement specified in `spikes/SPIKE-G/HARNESS.md`. See `spikes/SPIKE-G/results/RESULTS.md`.**
...
- ~~**Difficulty-grading coverage beyond whitewater (FR14b).** SPIKE-04 tested `whitewater:section_grade` and found it effectively absent in North America. Whether `sac_scale`, `trail_visibility`, `mtb:scale`, and `piste:difficulty` have usable coverage is an **open empirical question that has not been measured** — and v1.0 generalized the whitewater result to all technical terrain without testing it.~~ — **measured 2026-08-28 (SPIKE-C #170, ARCH Q13).** 57,422 ways across seven regions, including one region per mode where the mode is genuinely popular and the Tyrol as the schema homeland. **`piste:difficulty` is real everywhere** (86.4–100%, and North American piste density within 1.5–2.2× of the Tyrol's). **The three land schemas are not**, and the controls show why it is not a backlog: on Bentonville's own trail network `surface` is at 74.4% and `mtb:scale` at 0.76%. **The unexpected half is that thin coverage here fails *unsafely*** — a leg grade is worst-of its ways, so a worst-of over a sample understates by construction, and 16–32% of the grades this feature would print at real coverage rates are wrong and too easy. SPIKE-21's benign unknown-tag precedent does not transfer. FR14b narrowed to read-per-way-never-aggregate for land; Author declaration is the primary source, not the fallback.
...
- **Web reading surface and guest access.** FR132 puts full journey reading on the web. **SPIKE-F (#175) settled the reading half:** a Character *can* read a shared trip with no account, seeing the always-visible set only, and the Author-shareable link is a revocable share token that the reading view exchanges once for an opaque `HttpOnly` cookie (ARCH D59). What remains open is **guest *planning*** and how it interacts with FR60's stateless guest model — a separate surface from the share-token reading view.
